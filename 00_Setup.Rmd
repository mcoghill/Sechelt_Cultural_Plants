  ...
  ******************************************************************************
  ...
  if(!devtools::find_rtools()) {
    ...
    ****************************************************************************
    ...
    if(Sys.which("make") == "") 
      stop("rtools 4.0 is installed but not registered in your sytem PATH. Did
...
ls <- c("tidyverse", "raster", "sf", "terra", "foreach", "tools", "bcdata", 
        "bcmaps", "future.apply", "RStoolbox", "reticulate", "googledrive", 
        "stringi", "stars", "svSocket", "RODBC", "caret", "ranger", "GSIF",
        "MLmetrics", "mlr3verse", "mlr3fselect")
...
if(!"mlr3spatiotempcv" %in% installed.packages()[, "Package"] || 
   compareVersion(packageDescription("mlr3spatiotempcv")$Version, "0.0.0.9009") < 0)
  devtools::install_github("mlr-org/mlr3spatiotempcv", upgrade = TRUE, force = TRUE)

# Required minimum versions for certain packages
if(compareVersion(packageDescription("raster")$Version, "3.3-14") < 0) 
  devtools::install_github("rspatial/raster", upgrade = TRUE, force = TRUE)
