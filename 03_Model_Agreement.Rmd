...
Next, load the direcotries that will be used for this script.
...
dsmart_dirs <- file.path(
  AOI_dir, "1_map_inputs", paste0("dsmart_", c("site_ser", "struct_stg")))

covariates <- rast(c(
  list.files(file.path(covariate_dir, res_dir), 
             pattern = ".tif$", full.names = TRUE),
  list.files(file.path(dsmart_dir, "simple"), 
             pattern = ".tif$", full.names = TRUE),
  list.files(file.path(dsmart_dir, "output", "probabilities"), 
             pattern = ".tif", full.names = TRUE),
  file.path(dsmart_dir, "output", "realisations", "realisation_1.tif")))
...
prob_out_dir <- file.path(AOI_dir, "3_model_agreement", "pres_abs_agreement")
...
lapply(names(out), function(x) {
  lapply(names(out[[x]]), function(y) {
    lapply(names(out[[x]][[y]]), function(z) {
      write.csv(out[[x]][[y]][[z]], file.path(prob_out_dir, paste0(
...
cov_out_dir <- file.path(AOI_dir, "3_model_agreement", "cover_agreement")
...
full_out_dir <- file.path(AOI_dir, "3_model_agreement", "full_agreement")
