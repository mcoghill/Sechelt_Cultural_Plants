...
dem <- raster(file.path(cov_1m, "dem.tif"))
chm <- raster(file.path(cov_1m, "chm.tif"))

# Create 1m res polygon
dem_0 <- dem * 0
aoi_poly <- st_as_stars(dem_0) %>% 
...
if(!dir.exists(dl_path)) dir.create(dl_path)
...
trim <- foreach(i = urls, .combine = list) %do% {
  ...
  r <- raster(file.path(dl_path, basename(file_path_sans_ext(i))))
  r_bc <- projectRaster(r, res = c(25, 25), crs = paste0("+init=epsg:", epsg))
}
trim$fun <- mean
trim$na.rm <- TRUE
trim$tolerance <- 0.5

r_mos <- do.call(mosaic, trim)
...
lower_resolutions <- function(
  ...
  out_grids <- foreach(r = res_choices, .combine = c) %do% {
    ...
    if(!dir.exists(covariate_out_path)) dir.create(covariate_out_path)
    tool_list <- list(
