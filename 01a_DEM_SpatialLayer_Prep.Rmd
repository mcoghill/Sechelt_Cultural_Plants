...
# however, the extents do not match between the files, hence the list placement
# of each file
...
lidar <- lapply(raw, function(x) {
  ...
  # will indicate what rows/cols have data. The first colsum value is used to 
  # find the xmin coordinate whereas the last colsum value is used to find xmax.
  # This is reversed for finding y coordinates.
...
# will fit into
new_ext <- ext(
  c(xmin = min(sapply(lidar, xmin)), xmax = max(sapply(lidar, xmax)), 
...
lidar <- lapply(names(lidar), function(x) {
  expand(lidar[[x]], new_ext, overwrite = TRUE,
         filename = file.path(AOI_dir, "_original_files", x, paste0(x, ".tif")))
}) %>% do.call(c, .)
...
Resampling takes into account the neighbouring cells, so I don't see why it can't be used to downscale the DEM and CHM here.
...
for(r in c(1, 4, 5, 10, 25)) {
  ...
  lidar <- rast(list.files(raw_dir, pattern = ".tif$", 
...
dl_dir <- file.path(raw_dir, "TRIM")
trim_dir <- file.path(covariate_dir, "TRIM")

if(!dir.exists(dl_dir) || !dir.exists(trim_dir)) {
  dir.create(dl_dir, showWarnings = FALSE)
  dir.create(trim_dir, showWarnings = FALSE)
  dem_1 <- raster::raster(file.path(covariate_dir, "1m", "dem.tif"))
  ...
  trim <- lapply(urls, function(x) {
    ...
    return(rast(z))
  }) %>% Reduce(function(x, y) merge(x, y), .) %>% magrittr::set_names("dem")
  
  # raster package handles weird resolutions better and more accurately. Use
  # to project the merged raster to the new extent
  trim_proj <- raster::projectExtent(raster::raster(trim), crs(dem_1))
  
  # Use the largest of the resolutions returned (close to 25m here)
  res(trim_proj) <- max(res(trim_proj))
  
  # Convert back to terra object to project TRIM raster to
  trim_proj <- rast(trim_proj)
  
  # Resample 1m DEM to resolution of TRIM. This will become the cropping extent
  # and masking layer used later
  dem_1 <- rast(file.path(covariate_dir, "1m", "dem.tif"))
  dem_1_ext <- rast(ext(dem_1), crs = crs(dem_1))
  res(dem_1_ext) <- res(trim_proj)
  trim_mask <- resample(dem_1, dem_1_ext)
    
  # Crop and mask layer, save it to the TRIM folder
  trim_bca <- mask(
    project(trim, trim_mask, mask = TRUE), trim_mask, 
    filename = file.path(trim_dir, "dem.tif"), overwrite = TRUE)
...
dem_files <- grep("1m", list.files(covariate_dir, pattern = "dem.tif$", 
                                   full.names = TRUE, recursive = TRUE), 
                  value = TRUE, invert = TRUE)

covariates <- lapply(dem_files, function(x) 
  dem_derived_layers(x, saga_cmd = saga_cmd)) %>% 
  magrittr::set_names(basename(dirname(dem_files)))
...
# Flow is a bit weird - first, raster package is used to load DEM files. The 
# extents of those rasters is projected to lat/long, and the minimum resolution
# value from that is used to define the resolution for the whole pixel (this 
# is necessary because the projection causes unequl resolution values). Objects
# are then converted to terra objects for projection to lat/long. Any elevation 
# below 0 is changed to NA. Next, the raster package is used to change NA values  
# to -9999, again necessary for generating ClimateBC layers. Finally, the files 
# are written to .asc files. This type of file is not implemented (yet) in the
# terra package. This is a fairly quick function to run.
for(i in dem_files) {
  dem_bca <- raster::raster(i)
  dem_prj <- raster::projectExtent(dem_bca, crs("+init=epsg:4326"))
  res(dem_prj) <- min(res(dem_prj))
  dem_bca <- rast(i)
  dem_prj <- project(dem_bca, rast(dem_prj))
  ...
  raster::writeRaster(
    ...
    filename = file.path(
      clim_bc_dir, paste0("dem_", basename(dirname(i)), ".asc")))
  
  # Cleanup temp files
  suppressWarnings(tmpFiles(old = TRUE, remove = TRUE))
}

# Create climate layers for normal annual & seasonal 1961-1990, same for
# 1991-2010, for each resolution using the ClimateBC program. When that finishes, 
# come back here for renaming and saving the variables to their final location.
# ______________________________________________________________________________
# ______________________________________________________________________________
# ______________________________________________________________________________

# Folders indicate resolutions, subfolders are the data attribute
clim_res <- list.dirs(clim_bc_dir, full.names = TRUE, recursive = FALSE)
...
for(i in clim_res) {
  ...
  for(j in clim_sub) {
    ...
    # Remove layers from processing that are invalid (min values greater than 
    # max values and rasters that are all equal)
    clim_layers_valid <- as.data.frame(t(minmax(clim_layers))) %>% 
      rownames_to_column("id") %>% 
      dplyr::filter(rmin < rmax)
    
    clim_layers <- subset(clim_layers, clim_layers_valid$id)
    
    # Do the reprojection now
    p <- project(clim_layers, dem, mask = TRUE) %>% 
