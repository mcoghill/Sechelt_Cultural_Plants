...
for(r in c(1, 4, 5, 10, 25)) {
  ...
  if(r != 1) {
    ...
    lidar <- terra::resample(lidar, new_ext) %>% 
      writeRaster(filename = file.path(tempdir(), paste0("spat_", names(.), ".tif")),
                  overwrite = TRUE)
  }
  ...
  system2(saga_cmd, paste(
    ...
    "-OUTPUT", paste0(file.path(tempdir(), paste0(names(lidar), ".tif")), collapse = ";")
  ))
  
  # Reset the CRS data and write out
  out <- rast(file.path(tempdir(), paste0(names(lidar), ".tif")))
...
for(i in dem_files) {
  dem_bca <- rast(i)
  dem_prj <- raster::raster(project(dem_bca, "epsg:4326")) %>% 
    raster::writeRaster(
...
for(i in clim_res) {
  ...
  for(j in clim_sub) {
    ...
    clim_layers <- foreach(k = names(clim_layers), .combine = c) %do% {
      l <- writeRaster(subset(clim_layers, k), file.path(
        j, paste0(k, ".tif")), 
        overwrite = TRUE
      )
      return(sources(l)[, "source"])
    }
