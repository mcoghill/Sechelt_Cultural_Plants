...
if(ncell(dem_raw) > ncell(chm_raw)) {
  dem <- project(dem_raw, projection(CRS(paste0("+init=epsg:", epsg)))) #%>% 
  dem_ext <- ext(dem)
  res(dem) <- 4
  dem <- resample(dem, dem_ext)
    resample()
  , res = 1, 
...
for(i in clim_res) {
  ...
  for(j in clim_sub) {
    ...
    for(layer in names(s)) {
      writeRaster(subset(s, layer), file.path(
        dirname(clim_layers[1]), 
        paste0(layer, ".tif")), 
        overwrite = TRUE
      )
    }
    ...
    for(layer in clim_layers) {
      ...
      writeRaster(p, file.path(
        ...
        paste0(basename(j), "_", basename(layer))), 
        overwrite = TRUE
      )
    }
  }
