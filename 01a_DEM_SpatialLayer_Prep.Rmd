...
if(!dir.exists(dl_path) || dir.exists(trim_path)) {
  ...
  if(!dir.exists(trim_path)) dir.create(trim_path)
  dem_25 <- raster(file.path(covariate_file_path, "25m", "dem.tif"))
  ...
  trim <- foreach(i = urls, .combine = function(x, y) mosaic(x, y, fun = "mean") ) %do% {
    ...
    raster(file.path(dl_path, basename(file_path_sans_ext(i))))
  } %>% 
    projectRaster(dem_25) %>% 
    mask(dem_25)
  writeRaster(trim, file.path(trim_path, "dem.tif"), overwrite = TRUE)
}

low_res <- c(low_res, list.files(trim_path, pattern = "dem.tif", full.names = TRUE))
...
dem_files <- list.files(covariate_file_path, pattern = "dem.tif", full.names = TRUE, recursive = TRUE)
dem_files <- dem_files[basename(dirname(dem_files)) != "1m"]
clim_bc_path <- file.path(file.path(AOI.dir, "0_raw_inputs", "ClimateBC_Layers"))
if(!dir.exists(clim_bc_path)) dir.create(clim_bc_path, recursive = TRUE)
...
for(i in dem_files) {
  ...
  tool_list <- list(
    ...
    transform_grid = paste0(
      "<tool library='pj_proj4' tool='4' name='Coordinate Transformation (Grid List)'>
  ...
  sys_cmd <- paste(saga_cmd, "toolchains reproject_arc -dem", i)
  system(sys_cmd)
}
...
for(i in clim_res) {
  ...
  for(j in clim_sub) {
    ...
    dem <- raster(file.path(covariate_file_path, res, "dem.tif"))
    clim_layers <- list.files(j, full.names = TRUE, pattern = ".asc$")
    
    # Add CRS information and save as tif files
    # Determine total size of files to dynamically split reproject processing
    size <- length(clim_layers) * quantile(sapply(clim_layers, function(x) 
      file.size(x) / (1024 ^ 2), simplify = TRUE), probs = 0.75)
    memory <- memory.limit() - 1024
    
    if(size * 2 < memory) {
      s <- stack(clim_layers)
      crs(s) <- CRS("+init=epsg:4326")
      p <- projectRaster(s, dem) %>% 
        mask(dem)
      for(layer in names(p)) {
        writeRaster(subset(p, layer), file.path(
          covariate_file_path, res, 
          paste0(basename(j), "_", layer, ".tif")), 
          overwrite = TRUE
        )
      }
    } else {
      n_splits <- ceiling(size * 2 / memory)
      n_files <- ceiling(length(clim_layers) / n_splits)
      for(l in 1:n_splits) {
        m <- clim_layers[1:n_files + (n_files * (l - 1))]
        s <- stack(m[!is.na(m)])
        crs(s) <- CRS("+init=epsg:4326")
        beginCluster(parallel::detectCores())
        p <- projectRaster(s, dem) %>% 
          mask(dem)
        endCluster()
        for(layer in names(p)) {
