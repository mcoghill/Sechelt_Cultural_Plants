...
ls <- c("tidyverse", "raster", "xml2", "sf", "lubridate", "foreach", "rvest", "httr")
...
In this next chunk, you enter the only input data required for this sheet to function, the rest is (mostly) automated! Simply input the AOI, the choice of resolutions you want outputted, a projection code, and the file path for where SAGA is located.

The `remove` variable states which covariate layers will be removed at the end of the analysis (i.e.: intermediate layers). A list of layer names can be found by running the first "grid_list_filtered" variable, located after the last XML line.
...
In the next chunk, we want to specify which of the DEM files in the file list of the DEM's is the 1m raster. We are also preparing SAGA to run (note: not sure of the effect this has on other OS's).

```{r Specify 1m Raster}

covariate_file_path <- file.path(AOI.dir, "1_map_inputs", "covariates")
cov_1m <- file.path(covariate_file_path, "1m")
if(!dir.exists(cov_1m)) dir.create(cov_1m)

# Load files
dem_raw <- raster(file.path(AOI.dir, "_original_files", "dem", "w001001.adf"))
chm_raw <- raster(file.path(AOI.dir, "_original_files", "chm", "w001001.adf"))
...
# reset to have the correct resolution, crs, and extents
if(ncell(dem_raw) > ncell(chm_raw)) {
  dem <- projectRaster(
    dem_raw, crs = paste0("+init=epsg:", epsg), res = 1, 
    filename = file.path(cov_1m, "dem.tif"), overwrite = TRUE)
  chm <- projectRaster(
    chm_raw, dem, filename = file.path(cov_1m, "chm.tif"), overwrite = TRUE)
  
} else if(ncell(dem_raw) < ncell(chm_raw)) {
  chm <- projectRaster(
    chm_raw, crs = paste0("+init=epsg:", epsg), res = 1, 
    filename = file.path(cov_1m, "chm.tif"), overwrite = TRUE)
  dem <- projectRaster(
    dem_raw, chm, filename = file.path(cov_1m, "dem.tif"), overwrite = TRUE)
  
} else {
  NULL # Placeholder for now
}

dem_files <- list.files(covariate_file_path, pattern = "dem.tif$", full.names = TRUE, recursive = TRUE, ignore.case = TRUE)
dem_1m <- list.files(file.path(covariate_file_path, "1m"), pattern = "dem.tif", full.names = TRUE)

```

The next chunk is a function written to transform rasters using SAGA. Since SAGA is being used for most of the "heavy lifting", I thought I would script grid transforming here as well in order to be consistent. This function can be added into a loop if you wish so that you can transform multiple grids.

```{r Grid Transformation Function}

# Small edit to test github

```

This next chunk is a function which will start the pre-processing of a 1m resolution DEM into your desired DEM resolutions and places all separate DEM's into the AOI's DEM folder. This is following the examples set out by Lucas Jarron and Nicholas Coops, except this time R will be used to call SAGA GIS functions rather than using ArcGIS or QGIS.
...
lower_resolutions <- function(
  ...
  for(i in res_choices) {
     derive_dem_resolutions_xml <- paste(deparse(substitute(paste0("<?xml version='1.0' encoding='UTF-8'?>
     <toolchain saga-version='7.3.0'>
      ...
      <parameters>
        <option varname='GRID_SYSTEM' type='grid_system'>
         ...
         <input varname='DEM_in' type='grid' parent='GRID_SYSTEM'>
           <name>DEM Input</name>
         </input>
       </parameters>
        <tools>
         <tool library='statistics_grid' tool='1' name='Focal Statistics/Residual Analysis'>
           ...
           <option id='MODE'>0</option>
       	  <option id='RADIUS'>", round(i/2),"</option>
         ...
         <tool library='grid_gridding' tool='0' name='Shapes to Grid'>
           <output id='GRID'>DEM_out</output>
           <option id='FIELD'>", paste(basename(file_path_sans_ext(dem_1m)), "[Mean Value]"), "</option>
         ...
         <tool library='grid_tools' tool='17' name='Crop to Data'>
           <output id='OUTPUT'>DEM_out</output>
           <input id='INPUT'>DEM_out</input>
         </tool>

         <tool library='io_gdal' tool='2' name='Export GeoTIFF'>
            <input id='GRIDS'>DEM_out</input>
            <option id='FILE'>", normalizePath(file.path(out_path, "dem.tif")), "</option>
          </tool>
       </tools>
     </toolchain>"))), collapse = '')

     write_xml(read_xml(eval(parse(text = derive_dem_resolutions_xml))), file.path(dirname(saga_path), "tools", "toolchains", "DeriveDEMs.xml"))

     sys_cmd <- paste(saga_cmd, "toolchains Rescale",
                    "-DEM_in", dem_1m)
     system(sys_cmd)
     function_output <- c(function_output, file.path(out_path, "dem.tif"))
  }
...
dem_derived_layers <- function(
  ...
  if(is.null(out_path))
    out_path <- dirname(dirname(dem_input))
  ...
  covariate_out <- file.path(out_path, paste0(reference_res, "m"))
  if(!dir.exists(covariate_out)) dir.create(covariate_out, recursive = TRUE)
  ...
  centroid <- data.frame(x = mean(xmax(reference), xmin(reference)),
    ...
    st_coordinates() %>%
    as.data.frame() %>%
    mutate(X = as.character(round(X, digits = 2)),
           Y = as.character(round(Y, digits = 2))) %>%
    separate(X, into = c("Long_Hours", "Long_Mins"),
             sep = "\\.", remove = TRUE) %>%
    separate(Y, into = c("Lat_Hours", "Lat_Mins"),
             sep = "\\.", remove = TRUE) %>%
  ...
  filled_form <- set_values(
    form, phlong = centroid$Long_Hours, pmlong = centroid$Long_Mins,
    pdlat = centroid$Lat_Hours, pmlat = centroid$Lat_Mins, year = 2019,
    month = "Jan", day = 1, timezone = "PST|8", sunmoon = "Sunrise/set_yhtml1",
    ew = "West", sn = "North"
  )
  
  result <- submit_form(session, filled_form)
  response <- content(result$response)
  timetable <- html_table(html_node(response, "table")) %>%
    mutate_all(~ifelse(. %in% c("N/A", "null", ""), NA, .)) %>%
    na.omit() %>%
    mutate(Month = match(trimws(gsub("[[:digit:]]+", "", Date)), month.abb),
           Day = readr::parse_number(Date)) %>%
    mutate(rise = as_datetime(paste0(
      "2019-", Month, "-", Day, " ", hm(Sunrise)), tz = "Canada/Pacific"),
      set = as_datetime(paste0(
        "2019-", Month, "-", Day, " ", hm(Sunset)), tz = "Canada/Pacific")) %>%
  ...
  tool_list <- list(
    ...
    flow_accum = paste0(
      "<tool library='ta_hydrology' tool='1' name='Flow Accumulation (Recursive)'>
         ...
         <output id='FLOW_LENGTH'>flowlength4</output>
   ...
   # are calculated for a givenstudy area in order to limit the amount of
  ...
  for(p in 1:length(xml_layout)) {
    ...
    sys_cmd <- paste(saga_cmd, "toolchains Derived", cmd_text[[p]])
    system(sys_cmd)
  }
  
  # Remove intermediate files
  unlink(list.files(
    covariate_out, pattern = "ChannelNetwork|TCA|dem_preproc|sinkroute",
    full.names = TRUE)
  )
...
#transformed_raster <- saga_transform(dem_files[1], epsg_code = epsg)
#rescaled_dems <- dem_resolutions(dem_1m, res_choice)
covariate_list <- NULL
for(i in dem_files){
   dem_derivatives <- dem_derived_layers(i, individual_rasters = TRUE)
   covariate_list <- c(covariate_list, dem_derivatives)
}
```