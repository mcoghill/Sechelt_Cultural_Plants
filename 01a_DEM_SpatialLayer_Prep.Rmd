...
Below is the function which generates all SAGA covariate terrain layers used in this project. The first part of this chunk is the XML code for creating the tool chain. Within the function, this only needs to be ran once since it gets changed based on resolution with a much simpler function below it.
...
min_sunrise/max_sunset: Calculations for the minimum sunrise and maximum sunset times over the course of one year. This helps to keep the time for potential incoming solar radiation layer at reasonable levels so that it doesn't take forever creating the layer.
...
1. Fill Sinks in a DEM using the Sink Drainage Route Detection, and subsequent Sink removal tools: For filling sinks in the DEM and only returning a DEM

2. Slope, Aspect, Curvature: returns Slope, Aspect, General Curvature, and Total Curvature rasters

3. Flow Accumulation (Recursive): Creates the "total catchment area" (TCA) intermediate layer. This layer is only used in the generation of the TWI, and not in the final raster stack.
...
This function requires that you specify an input file DEM either as a raster file or a file path to a raster file. The files will be written in a path either specified or relative to the input DEM.
  ...
  for(j in clim_sub) {
    ...
    for(layer in clim_layers) {
      p <- rast(layer) %>% 
        project(dem) %>% 
        mask(dem) %>% 
        magrittr::set_names(tolower(paste0(basename(j), "_", file_path_sans_ext(basename(layer)))))
