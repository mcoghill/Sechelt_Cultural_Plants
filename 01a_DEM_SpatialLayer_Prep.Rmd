...
lidar <- try(stack(file.path(cov_1m, "dem.tif"), 
                   file.path(cov_1m, "chm.tif")))
if(class(lidar) %in% "try-error") {
  stop("There is something wrong, please check the rasters to ensure they may be stacked")
}

lidar_layers <- rast(list.files(file.path(AOI.dir, "_original_files"), 
                           pattern = "chm.tif|dem.tif", 
                           full.names = TRUE, recursive = TRUE))
crs(lidar_layers) <- projection(CRS(paste0("+init=epsg:", epsg)))
for(i in names(lidar_layers)) {
  writeRaster(subset(lidar_layers, i), 
...
low_res <- foreach(grid = lidar_layers@ptr$filenames, .combine = c) %do% {
  lower_resolutions(
    grid_1m = grid, 
