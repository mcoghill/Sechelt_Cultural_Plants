...
ls <- c("tidyverse", "raster", "xml2", "sf", "lubridate", "foreach", "rvest", "httr", "tools")
...
if(!dir.exists(cov_1m)) dir.create(cov_1m, recursive = TRUE)
...
} else if(ncell(dem_raw) < ncell(chm_raw) || any(
  ncol(chm_raw) != ncol(dem_raw), 
  nrow(chm_raw) != nrow(dem_raw),
  res(chm_raw) != res(dem_raw), 
  res(chm_raw) != c(1, 1),
  !compareCRS(chm_raw, dem_raw), 
  !compareCRS(chm_raw, CRS(paste0("+init=epsg:", epsg))))) {
...
lidar <- try(stack(chm_raw, dem_raw))
if(class(lidar) %in% "try-error") {
  stop("There is something wrong, please check the rasters to ensure they may be stacked")
}

lidar_layers <- list.files(cov_1m, pattern = "chm.tif|dem.tif", full.names = TRUE)
...
lower_resolutions <- function(
  ...
  out_grids <- foreach(r = res_choices, .combine = c) %do% {
    ...
    for(p in 1:length(xml_layout)) {
      ...
      sys_cmd <- paste(saga_cmd, "toolchains Downscale", cmd_text[[p]])
      system(sys_cmd)
    }
    
    # Remove intermediate files
    unlink(list.files(
      covariate_out_path, pattern = "grid_smooth|grid_empty|points_empty|points_filled|grid_downscaled",
      full.names = TRUE)
    )
...
low_res <- foreach(grid = lidar_layers, .combine = paste) %do% {
  lower_resolutions(
    grid_1m = grid, 
...
dem_files <- grep("dem.tif", low_res, value = TRUE)

covariate_list <- foreach(i = dem_files) %do% {
  dem_derived_layers(i, individual_rasters = TRUE)
}
