...
for(i in clim_res) {
  ...
  for(j in clim_sub) {
    ...
    dem <- grep(res, dem_files, value = TRUE)
    clim_layers <- list.files(j, full.names = TRUE, pattern = ".asc$")
    for(k in clim_layers) {
      l <- raster(k)
      crs(l) <- CRS("+init=epsg:4326")
      writeRaster(l, file.path(dirname(k), paste0(basename(file_path_sans_ext(k)), ".tif")), 
                  overwrite = TRUE)
    }
    system(paste(saga_cmd, "pj_proj4 3", 
                 "-SOURCE", paste(clim_layers[1:2], collapse = ";"),
                 "-TARGET_DEFINITION 1", 
                 "-TARGET_TEMPLATE", dem,
                 "-CRS_PROJ4", paste0('"', projection(raster(dem)), '"'),
                 "-GRIDS", paste0(file.path(
                   covariate_file_path, res, 
                   paste0(basename(dirname(clim_layers)), "_", basename(clim_layers[1:2]))), 
                   collapse = ";")))
  }
}
