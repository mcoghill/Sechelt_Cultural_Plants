...
for(i in dem_files) {
  ...
  sys_cmd <- paste(saga_cmd, "toolchains polygonize -dem", i)
  system(sys_cmd)
}
...
get_bc_shapes <- function(in_aoi = NULL, 
  ...
  fix_geoms <- function(x) {
    if(any(!st_geometry_type(x) %in% c("POLYGON", "MULTIPOLYGON"))) {
      bec_geom <- st_geometry(x)
      new_geom <- foreach(geom = bec_geom) %do% {
        if(!st_geometry_type(geom) %in% c("POLYGON", "MULTIPOLYGON")) {
          for(f in 1:length(geom)) {
            if(length(geom) > f) {
              if(!st_geometry_type(geom[[f]]) %in% c("POLYGON", "MULTIPOLYGON")) {
                geom[[f]] <- NULL
              }
            }
          }
          geom <- st_multipolygon(geom)
        } else if(st_geometry_type(geom) == "POLYGON") {
          geom <- st_multipolygon(list(geom))
        }
        geom
      } %>% 
        st_sfc()
...
in_aoi <- list.files(shape_dir, pattern = "aoi.gpkg", recursive = TRUE, full.names = TRUE)
