...
In order to get accurate polygon edges for each raster resolution, they need to be individually created and saved. The polygonization of a raster will take place using SAGA GIS (chosen for speed). It first replaces values in a raster with 0's, and then polygonizes the "grid classes" (of which there would only be a single class, 0). The output raster is saved as a geopackage.
...
get_bc_shapes <- function(in_aoi, 
  ...
  } else if(is.character(in_aoi)) { study_area <- st_read(in_aoi, quiet = TRUE)
  ...
  suppressMessages(suppressWarnings({
    ...
    st_write(bec_sf, file.path(out_path, "bec.gpkg"), delete_dsn = TRUE, quiet = TRUE)
    ...
    st_write(vri, file.path(out_path, "vri_full.gpkg"), delete_dsn = TRUE, quiet = TRUE)
    st_write(dplyr::select(vri, "BCLCS_LEVEL_2"), file.path(out_path, "vri_trees.gpkg"), # Treed sites
    ...
    st_write(cutblocks, file.path(out_path, "young_cutblocks.gpkg"), 
             delete_dsn = TRUE, quiet = TRUE)
    ...
    tem <- bcdc_query_geodata("0a83163b-a62f-4ce6-a9a1-21c228b0c0a3", crs = epsg) %>%
      ...
      collect() %>% 
      {if(nrow(.) > 0) st_intersection(., study_area) else .}
    st_write(tem, file.path(out_path, "tem.gpkg"), 
             delete_dsn = TRUE, quiet = TRUE)
    ...
    st_write(waterbodies, file.path(out_path, "water.gpkg"), 
             delete_dsn = TRUE, quiet = TRUE)
    ...
    roads <- bcdc_query_geodata("bb060417-b6e6-4548-b837-f9060d94743e", crs = epsg) %>% 
      bcdata::filter(
        BBOX(st_bbox(study_area), crs = epsg),
        ROAD_NAME_ID > 0) %>%
      collect() %>% 
    ...
    st_write(road_merge, file.path(out_path, "road_network.gpkg"), 
             delete_dsn = TRUE, quiet = TRUE)
    ...
    parallel::stopCluster(cl)
    st_write(fires, file.path(out_path, "fires.gpkg"), 
             delete_dsn = TRUE, quiet = TRUE)
  }))
