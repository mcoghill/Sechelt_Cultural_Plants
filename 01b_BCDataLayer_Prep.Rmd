...
suppressMessages(suppressWarnings({
  lapply(c("bcdata", "bcmaps", "tidyverse", "terra", "future.apply"), 
...
for(i in dem_files) {
  ...
  # Convert DEM raster to polygon and save as .gpkg by converting to sfc object
...
A single function is used to download all layers listed above. If multiple AOI's are provided, the layer is downloaded once and then clipped to the respective AOI.

```{r Function}

get_bc_shapes <- function(in_aoi, out_dir, epsg = 3005) {
  
  #### Input checks
  ## Restrict in_aoi to being an sf, sfc, or list of sf or sfc objects 
  ## (i.e.: polygons only). sfg objects not allowed since they have no CRS
  ## information. STILL NEED CASE WHEN THERE ARE MULTIPLE OBJECTS, ex: an sf
  ## dataframe with more than one row. Right now, this works when it is only a
  ## single row. We only really want a single sfc polygon out, so maybe an
  ## st_combine call?
  if(missing(in_aoi)) stop("'in_aoi' is missing with no default")
  if(missing(out_dir)) stop("'out_dir' is missing with no default")
  if(length(out_dir) > 1) stop("Only one file path allowed")
  if(!is.numeric(epsg)) stop("\rError in epsg: Not a numerical EPSG code")
  ...
  if(all(sapply(in_aoi, function(x) 
    ...
    if(all(sapply(in_aoi, function(x) {
      ...
      stop("Not all listed sf features are of POLYGON or MULTIPOLYGON class")
    ...
    stop("Some list elements are not valid sf or sfc obects.")
  
  # Set out_dir variable
  if(length(study_area) > 1 || names(study_area)[1] != rand_name) {
    out_dir <- as.list(file.path(out_dir, names(in_aoi))) %>% 
      magrittr::set_names(names(study_area))
  } else out_dir <- as.list(out_dir) %>% magrittr::set_names(names(study_area))
  sapply(out_dir, dir.create, showWarnings = FALSE, recursive = TRUE)
  ...
  suppressMessages({
    ...
    tem <- sapply(names(study_area), function(x) {
      if(nrow(tem) > 0) {
        ...
        sf::st_write(tem, file.path(out_dir[[x]], "tem.gpkg"),
                     delete_dsn = TRUE, quiet = TRUE)
      } else tem <- tem
    ...
    lapply(names(tem), function(x) {
      ...
      if(nrow(tem[[x]]) > 0 && 
         all(unique(bec_sf[[x]]$MAP_LABEL) %in% 
             unique(tem[[x]]$BIOGEOCLIMATIC_LBL))) {
        ...
        } else bec_sf[[x]] <- dplyr::rename(bec_fix, 
                                            MAP_LABEL = BIOGEOCLIMATIC_LBL)
      }
      sf::st_write(bec_sf[[x]], file.path(out_dir[[x]], "bec.gpkg"), 
                   delete_dsn = TRUE, quiet = TRUE)
    })
    ...
    vri <- sapply(names(study_area), function(x) {
      if(nrow(vri) > 0) {
        ...
        sf::st_write(vri, file.path(out_dir[[x]], "vri_full.gpkg"), 
                     delete_dsn = TRUE, quiet = TRUE)
        sf::st_write(dplyr::select(vri, "BCLCS_LEVEL_2"), 
                     file.path(out_dir[[x]], "vri_trees.gpkg"), # Treed sites
                     delete_dsn = TRUE, quiet = TRUE)
      } else vri <- vri
      return(vri)}, simplify = FALSE, USE.NAMES = TRUE)
    
    # Download recent cutblocks layer. Uses date filter which filters cutblock 
    # ages less than 20 years, or 7305 days
    ...
    cutblocks <- sapply(names(study_area), function(x) {
      if(nrow(cutblocks) > 0) {
        ...
        sf::st_write(cutblocks, file.path(out_dir[[x]], "young_cutblocks.gpkg"), 
                     delete_dsn = TRUE, quiet = TRUE)
    ...
    waterbodies <- sapply(names(study_area), function(x) {
      if(nrow(waterbodies) > 0) {
        waterbodies <- sf::st_intersection(waterbodies, study_area[[x]]) %>% 
          st_force_poly()
        sf::st_write(waterbodies, file.path(out_dir[[x]], "water.gpkg"), 
             delete_dsn = TRUE, quiet = TRUE)
    ...
    roads <- sapply(names(study_area), function(x) {
      if(nrow(roads) > 0) {
        ...
        sf::st_write(roads, file.path(out_dir[[x]], "road_network.gpkg"), 
                     delete_dsn = TRUE, quiet = TRUE)
      } else roads <- roads
    ...
    fires <- sapply(names(study_area), function(x) {
      if(nrow(fires) > 0) {
        fires <- sf::st_intersection(fires, study_area[[x]]) %>% 
          ...
          st_force_poly()
        sf::st_write(fires, file.path(out_dir[[x]], "fires.gpkg"), 
                     delete_dsn = TRUE, quiet = TRUE)
      } else fires <- fires
...
The AOI's are read into R and then converted to sfc objects (not necessary, just simpler for me to think about). The function is then ran to process each file. Improvements may be made where files don't get saved in the function but rather get loaded into the R environment, as well as perhaps placing the above function into a .R file of its own, and finally applying filters for which layers I would care to download from the ones coded in there.
...
bc_shapes <- get_bc_shapes(aoi_shapes, out_dir = shp_dir)
