...
As of January 2021, the terra package doesn't play well with the sf package. This small function is a quick helper from the terra package author to convert SpatVect objects to sf objects
...
aoi_shapes <- lapply(dem_files, function(i) {
  ...
  dem_shp_all <- as.polygons(dem, trunc = TRUE, dissolve = TRUE, values = TRUE) 
  
  # Small disjointed polygons have 5 points. Filter those, create a polygon,
  # buffer it slightly, and union them with the main polygon
  dem_small <- terra::geom(dem_shp_all, df = TRUE) %>% 
    ...
    {if(nrow(.) > 0) {
      ungroup(.) %>% 
        ...
        sf::st_sfc() %>% 
        sf::st_as_sf() %>% 
        dplyr::rename(geometry = names(.)) %>% 
        sf::st_set_crs(crs(dem)) %>% 
        sf::st_buffer(res(dem)[1] / 10, nQuadSegs = 0) %>% 
        dplyr::mutate(dem = 0)
    } else NULL}
  
  dem_shp <- st_as_sf(dem_shp_all) %>% 
    rbind(dem_small) %>% 
  ...
  # Might need to do something with holes by removing them from a total geometry
  # If so, think of the sf::st_difference() function
  
  return(dem_shp)
}) %>% do.call(rbind, .) %>% tibble::remove_rownames()

aoi_sfc <- st_geometry(aoi_shapes)
aoi_sfc_lonlat <- st_transform(aoi_sfc, 4326)
aoi_sf_lonlat <- sf::st_transform(aoi_shapes, 4326)
aoi_sfc_bad <- c(aoi_sfc[1:3], aoi_sfc_lonlat[4:6]) # Don't think I can do anything about this...
aoi_sfc_utm <- st_transform(aoi_sfc, 32610)
...
get_bc_shapes <- function(in_aoi, id_col) {
  ...
  if(nrow(in_aoi) != length(unique(dplyr::pull(in_aoi, id_col))))
    stop("Something is wrong with the 'id_col' variable")
  ...
  if(!all(st_intersects(st_centroid(in_aoi), bc_boundary, sparse = FALSE)))
    warning("One or some of the input polygons may be outside of the BC provincial boundary. 
            \rIt may be worth it to check your inputs if the results are not expected.
            \rIgnore this warning if you're confident things are as they should be.")
  ...
  suppressMessages({
    ...
    clusterExport(cl, c("box", "links", "epsg", "in_aoi"))
    ...
    if(sf::st_is_longlat(in_crs)) {
      clusterExport(cl, c("dls_full", "in_crs"))
