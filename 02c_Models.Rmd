...
site_series <- file.path(ss_dir, paste0("site_series_", ss, ".tif"))
covariates <- rast(c(terrain_covariates, sentinel_covariates, climate_covariates, site_series))

# Attributed point data
pres_abs <- sapply(st_layers(file.path(input_dir, "pres_abs.gpkg"))$name, function(x) {
...
cover <- sapply(st_layers(file.path(input_dir, "cover.gpkg"))$name, function(x) {
...
for(i in 1:length(cover_models)) {
  ...
  out <- predict_landscape(cover_models[[i]], cov_important, tilesize = 500,
                           outDir = tile_dir)
  writeRaster(out, file.path(output_dir, "cover_maps", paste0(names(pres_models)[i], "_cover.tif")))
}
