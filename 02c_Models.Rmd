...
dsmart_site_ser_dir <- file.path(AOI_dir, "1_map_inputs", "dsmart_site_ser")
dsmart_struct_stg_dir <- file.path(AOI_dir, "1_map_inputs", "dsmart_struct_stg")
...
terrain_covariates <- grep(pattern = "normal|sentinel2", list.files(
  file.path(covariate_dir, res_dir), full.names = TRUE, pattern = ".tif$"), 
...
sentinel_covariates <- grep(file.path(covariate_dir, res_dir, s_pattern), 
                            list.files(file.path(covariate_dir, res_dir), full.names = TRUE), 
                            value = TRUE, perl = TRUE)

climate_covariates <- if(any(climate_vars)) {
  grep(file.path(covariate_dir, res_dir, c_pattern),
       list.files(file.path(covariate_dir, res_dir), full.names = TRUE), 
       value = TRUE, perl = TRUE)
} else NULL

bgc_layer <- if(any(bgc)) {
  file.path(dsmart_site_ser_dir, "inputs", "bgc.tif")
} else NULL
...
if(!all(is.na(ss))) {
  site_series <- c(
    ...
    rast(file.path(dsmart_site_ser_dir, "output", "realisations", "realisation_1.tif")) %>% 
