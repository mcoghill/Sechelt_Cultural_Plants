...
saveRDS(pres_models, file.path(output_dir, "pres_abs_maps", "cover_models.RDS"))
...
lapply(seq_along(pres_models$learners), function(x) {
  predict_landscape_mlr3(
    learner = pres_models$learners[[x]],
    task = pres_models$tasks[[x]], 
    type = "prob",
    covariates = cov_raster,
    tilesize = 500, 
    outDir = tile_dir
  )
})
