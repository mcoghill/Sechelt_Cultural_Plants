...
covariates <- c(rast(c(terrain_covariates, sentinel_covariates, climate_covariates)),
  site_series)

# Find the best masking layer to use in the predict_landscape algorithm
mask_layer <- least_na(covariates)
...
# Write input data
dir_out <- file.path(output_dir, "pres_abs_full_predictions")
dir.create(dir_out, showWarnings = FALSE, recursive = TRUE)
lapply(names(pres_abs_full), function(x) {
  y <- st_as_sf(pres_abs_full[[x]], coords = c("X", "Y"), crs = crs(covariates))
...
pres_performance <- sapply(names(full_pres_models$learners), function(x) {
  
  dir_out <- file.path(output_dir, "pres_abs_full_predictions", x)
  dir.create(dir_out, showWarnings = FALSE)
  
  tune_results <- data.frame(
    filter_fraction = sapply(full_pres_models$learners[[x]]$archive()$tune_x, function(x)
    x[[1]]), 
    classif.ce = full_pres_models$learners[[x]]$archive()$classif.ce
  )
  
  confusion <- as.data.frame(
    full_pres_models$learners[[x]]$model$tuning_instance$best()$prediction()$confusion)
  
  # There were no rep and fold ID's in the table so I had to create them from scratch
  folds_reps <- sapply(
    full_pres_models$learners[[x]]$model$tuning_instance$best()$data$prediction, 
    function(x) 
  ...
  test_data <- as.data.frame(
    full_pres_models$learners[[x]]$model$tuning_instance$best()$data$resampling[[1]]$instance) %>% 
    mutate(row_id = paste0(row_id, "_", fold, "_", rep)) %>% 
    merge(cbind(
      full_pres_models$learners[[x]]$model$tuning_instance$best()$prediction()$data$tab %>% 
        mutate(row_id = paste0(row_id, "_", row_ids)),
      full_pres_models$learners[[x]]$model$tuning_instance$best()$prediction()$data$prob))
  
  imp <- data.frame(full_pres_models$learners[[x]]$learner$model$classif.ranger$model$variable.importance) %>% 
    rownames_to_column() %>% 
    dplyr::rename_at(vars(names(.)), ~c("layer", "importance")) %>% 
    arrange(desc(importance))
  
  out <- utils::capture.output(full_pres_models$learners[[x]]$learner$model$classif.ranger$model)
  best_model <- full_pres_models$learners[[x]]$learner$model$classif.ranger$model
  ...
  write.csv(imp, file.path(dir_out, paste0(x, "_pres_abs_var_importance.csv")), 
            row.names = FALSE)
  write.csv(pres_abs_full[[x]], file.path(dir_out, paste0(x, "_pres_abs_input_data.csv")), 
            row.names = FALSE)
...
# We might only be interested in the best results, so filter down the entire
# model set before predicting to extract the best models only
best_models <- data.frame(classif.ce = sapply(pres_performance, function(x) {
  min(x$tune_results$classif.ce)
})) %>% 
  rownames_to_column("model")

best_model_list <- lapply(c("CORNCAN", "RUBUSPE"), function (x) {
  best_models %>% 
    dplyr::filter(startsWith(model, x)) %>% 
    dplyr::slice(which.min(classif.ce))
})

# Run the map predictions
# Increase memory limit artificially
memory.limit(size = memory.limit() * 2)

pres_abs_predictions <- sapply(names(full_pres_models$learners), function(x) {
  
  dir_out <- file.path(output_dir, "pres_abs_full_predictions", x)
  dir.create(dir_out, showWarnings = FALSE)
  cov_sub <- subset(covariates, full_pres_models$tasks[[x]]$col_roles$feature)
  
  for(i in 1:nrow(mask_layer)) {
    if(mask_layer[i, "layer"] %in% names(cov_sub)) {
      pred_mask <- mask_layer[i, "layer"]
      break
    }
  }
  
  out <- predict_landscape_mlr3(
    learner = full_pres_models$learners[[x]],
    task = full_pres_models$tasks[[x]], 
    type = "prob",
    covariates = cov_sub,
    tilesize = 500, 
    outDir = tile_dir, 
...
pres_models <- model_gen_mlr3(traindat = pres_abs, target = "Pres", 
