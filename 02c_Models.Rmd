...
for(i in 1:length(pres_abs)) {
  ...
  pres_performance <- sapply(names(pres_models$learners), function(x) {
    ...
    folds_reps <- sapply(
      ...
      function(y) 
        nrow(y$test$data$tab))
    
    row_ids <- foreach(j = 1:length(folds_reps), .combine = c) %do% {
      folds <- as.character(j)
      if(nchar(folds) > 1) folds <- substring(folds, 2)
      if(folds == "0") folds <- "10"
      
      reps <- ifelse(j %in% 1:10, 1, ifelse(j %in% 11:20, 2, ifelse(
        j %in% 21:30, 3, ifelse(j %in% 31:40, 4, 5))))
      return(rep(paste0(folds, "_", reps), length.out = folds_reps[j]))
    }
