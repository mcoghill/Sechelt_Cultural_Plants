...
include_bgc <- FALSE
use_caret <- TRUE
run <- "struct_stg" # option of "struct_stg" or "site_ser" or "struct_stg_simple"
...
polygons <- terra::vect(file.path(paste0(dsmart_dir, "_", run), "inputs", paste0(
...
composition <- read.csv(file.path(paste0(dsmart_dir, "_", run), "inputs", paste0(
  "dsmart_", run, "_composition.csv"))) %>% 
    dplyr::select(-Subzone)
observations <- read.csv(file.path(paste0(dsmart_dir, "_", run), "inputs", paste0(
...
if(run == "site_ser") {
  ...
  tem <- terra::vect(file.path(dsmart_dir, "inputs", "dsmart_polygons.gpkg"))
...
outputdir <- file.path(AOI_dir, "1_map_inputs", paste0("dsmart_", run))
