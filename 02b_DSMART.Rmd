...
ls <- c("devtools", "sf", "raster", "fasterize", "rgdal", "caret", "randomForest", 
        "tidyverse", "terra", "foreach", "GSIF")
new.packages <- ls[!(ls %in% installed.packages()[, "Package"])]
if(length(new.packages))
  install.packages(new.packages)
#devtools::install_bitbucket("brendo1001/dsmart", upgrade = "always")
...
# for(i in lsf.str(envir = asNamespace("rdsmart"), all = TRUE)) {
#   if(!file.exists(file.path("./_functions", paste0(i, ".R"))))
#     dump(i, file = file.path("./_functions", paste0(i, ".R")))
#   source(file.path("./_functions", paste0(i, ".R")))
# }
...
# Read all other variables needed by function arguments
# rate = 15
# reals = 5
# n.realisations <- reals
...
site_series <- dsmart(covariates = dsmart_covariates, 
                      ...
                      outputdir = file.path(AOI_dir, "1_map_inputs", "dsmart"), 
                      cpus = detectCores() - 1, 
                      args.model = list(trControl = fit_control,  
                                        tuneGrid = tune_grid))
