...
polygons <- terra::vect(file.path(dsmart_dir, "inputs", "dsmart_polygons.gpkg"))
...
outputdir <- file.path(AOI_dir, "1_map_inputs", "dsmart2")
...
site_series <- rast(list.files(dsm$summarise$locations$mostprobable, 
                          pattern = "mostprob_01_class", full.names = TRUE)) %>% 
  magrittr::set_names("site_series_dsmart") %>% 
  writeRaster(file.path(outputdir, "site_series_dsmart.tif"), 
              overwrite = TRUE, wopt = list(datatype = "INT2S"))
