...
check <- lapply(cov_res, function(x) {
  res_def <- suppressWarnings(as.numeric(x))
  covariate_subdir <- if(!is.na(res_def)) paste0(res_def, "m") else x
  all_covs <- list.files(file.path(covariate_dir, covariate_subdir), 
