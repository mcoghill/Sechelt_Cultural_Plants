...
suppressMessages(suppressWarnings({
  lapply(c("tidyverse", "tools", "terra", "sf", "mlr3verse", "foreach", "corrplot"), 
...
mask_layers <- lapply(covariates, function(x) {
  ...
  remove_rownames() %>% 
  dplyr::filter(data_cells > quantile(data_cells, 0.01))

cov_atts <- mask_layers$layer
...
pres_abs <- sapply(c("Corn can", "Rubu spe"), function(x) {
  st_read(file.path(input_dir, "pres_abs.gpkg"), layer = x, quiet = TRUE) %>% 
    ...
    dplyr::select(Pres, cov_atts[cov_atts %in% names(.)], X, Y)
...
for(j in names(covariates)) {
  ...
  pres_abs_flt <- lapply(pres_abs, function(x) 
    dplyr::select(x, any_of(c("Pres", names(covariates_flt), "X", "Y"))) %>% 
      drop_na())
  
  # Filter down the covariates to only include the ones that weren't filtered 
  # out based on the mask_layer variable
  covariates_flt <- subset(covariates_flt, names(pres_abs_flt[[1]])
                           [!names(pres_abs_flt[[1]]) %in% c("Pres", "X", "Y")])
