...
models <- sapply(names(covariates), function(j) {
  ...
  sp_models <- lapply(1:length(pres_abs_flt), function(i) {
    ...
    pres_model <- model_gen_mlr3_binary_simple(traindat = pres_abs_flt[[i]], target = "Pres",
                                  ...
                                  outDir = dir_out)
    # 
    # covs <- subset(covariates_flt, names(pres_model$variable.importance))
    # wopt <- list(names = "TRUE")
    # pred <- terra::predict(
    #   covs, pres_model, na.rm = TRUE,
    #   fun = function(model, ...) predict(model, verbose = FALSE, ...)$predictions,
    #   index = 1L,
    #   filename = file.path(dir_out, paste0(wopt$names, ".tif")), overwrite = TRUE,
    #   wopt = wopt)
...
Now it's just a matter of tabulating the results of each model run
...
# all_preds <- lapply(sp, function(x) {
#   lapply(cov_grps, function(y) {
#     mod_runs <- list.dirs(file.path(output_dir, "pres_abs_predictions", x, y), recursive = FALSE)
#     lapply(mod_runs, function(z) {
#       f <- file.path(z, "best_model.rds")
#       if(file.exists(f)) {
#         model <- readRDS(f)
#         covs <- subset(covariates[[y]], names(model$variable.importance))
#         wopt <- list(names = "TRUE")
#         pred <- terra::predict(
#           covs, model, na.rm = TRUE,
#           fun = function(model, ...) predict(model, verbose = FALSE, ...)$predictions,
#           index = 1L,
#           filename = file.path(z, paste0(wopt$names, ".tif")), overwrite = TRUE,
#           wopt = wopt)
#       } else NULL
#     })
#   })
