...
for(j in names(covariates)) {
  ...
  for(i in 1:length(pres_abs_flt)) {
    ...
    pres_models <- model_gen_mlr3(traindat = pres_abs_flt[i], target = "Pres", 
                                  ...
                                  feature_selection = "rfe", type = "prob")
...
best_model_sets <- bind_rows(lapply(c("Corn can", "Rubu spe"), function(x) {
  ...
  result_list <- bind_rows(lapply(sp_dirs, function(y) {
    r <- read.csv(file.path(y, list.files(y, pattern = "_tune_results.csv$"))) %>% 
      dplyr::slice(which.min(classif.ce)) %>% 
      mutate(model = basename(y)) %>% 
      dplyr::select(model, classif.ce)
    t <- read.csv(file.path(y, list.files(y, pattern = "_model_metrics.csv$"))) %>% 
      pivot_wider(names_from = "metric", values_from = "result")
    r <- cbind(r, t)})) %>% 
    dplyr::arrange(classif.ce)
  write.csv(result_list, file.path(
    output_dir, "pres_abs_predictions", x, paste0("model_performance_", x, "_", res_dir, ".csv")),
    row.names = FALSE)
  dplyr::mutate(result_list, species = x)})) %>% 
  dplyr::relocate(species, model)

write.csv(best_model_sets, file.path(
  output_dir, "pres_abs_predictions", paste0("model_performance_index", "_", res_dir, ".csv")), 
  row.names = FALSE)
