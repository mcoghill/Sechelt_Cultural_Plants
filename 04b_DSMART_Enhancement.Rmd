...
out_dir <- file.path(AOI_dir, "1_map_inputs", paste0("2020_dsmart_", run))
...
terrain_covariates <- grep(pattern = "normal|sentinel2", list.files(
  file.path(covariate_dir, res_dir), full.names = TRUE, pattern = ".tif$"), invert = TRUE, 
  value = TRUE)
sentinel_covariates <- list.files(file.path(covariate_dir, res_dir), 
                                  pattern = "^sentinel2.*.2019.tif$", full.names = TRUE)
climate_covariates <- list.files(file.path(covariate_dir, res_dir), 
...
if(include_bgc) {
  bgc <- st_read(file.path(shapes_dir, res_dir, "bec.gpkg"), quiet = TRUE) %>% 
...
if(run == "site_ser") {
  dir.create(file.path(out_dir, "simple"), showWarnings = FALSE)
  ...
  tem_rast <- terra::rasterize(tem, covariates[[1]], field = "MapUnit1_num", 
                               filename = file.path(out_dir, "simple", "site_series_simple.tif"), 
                               overwrite = TRUE)
  ...
  tem_props <- foreach(i = unique(tem_remake$MapUnit), .combine = c) %do% {
    tem_remake[tem_remake$MapUnit == i] %>% 
      ...
      rasterize(covariates[[1]], field = paste0("probability_", i), overwrite = TRUE, 
                filename = file.path(out_dir, "simple", paste0("probability_", i, ".tif")))
  }
}
