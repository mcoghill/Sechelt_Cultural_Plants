...
covariates <- rast(c(
  list.files(file.path(covariate_folder, res_folder), 
...
out <- sapply(c("Corn can", "Rubu spe"), function(i) {
  ...
  model_stack <- rast(
    sapply(dir(pred_folder, pattern = i, full.names = TRUE), list.files, 
  ...
  mod_modified <- sapply(prob_cutoff, function(j) {
    ...
    agreement <- app(
      ...
      filename = file.path(out_folder, paste0(
...
lapply(names(out), function(x) {
  lapply(names(out[[x]]), function(y) {
    lapply(names(out[[x]][[y]]), function(z) {
      write.csv(out[[x]][[y]][[z]], file.path(out_folder, paste0(
