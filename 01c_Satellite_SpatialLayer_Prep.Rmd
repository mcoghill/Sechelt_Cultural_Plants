...
cov_res <- grep("1m$", list.dirs(cov_dir, recursive = FALSE), invert = TRUE, value = TRUE)
dem_list <- sapply(cov_res, list.files, pattern = "dem.tif", full.names = TRUE,
                   USE.NAMES = FALSE)

# Define AOI shape to use - defined by largest BBOX area
shp_dir <- file.path(AOI_dir, "0_raw_inputs", "base_layers")
shp_res <- list.dirs(shp_dir, recursive = FALSE)
shp_list <- sapply(shp_res, list.files, pattern = "aoi.gpkg", full.names = TRUE,
                   USE.NAMES = FALSE)
shp <- shp_list[which.max(sapply(shp_list, function(x) 
  st_read(x, quiet = TRUE) %>% sf::st_bbox() %>% st_as_sfc() %>% st_area()))]
...
AOI_bbox <- st_read(shp, quiet = TRUE) %>%
  st_bbox() %>%
  st_as_sfc() %>%
  st_buffer(250)
