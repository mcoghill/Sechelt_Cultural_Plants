...
ls <- c("raster", "RStoolbox", "reticulate", "terra", "sf",
...
bands_to_indices <- function(
  ...
  suppressMessages(suppressWarnings({
    ...
    # Change band names to be more descriptive and match names in RStoolbox package
    band_lookup <- tribble(
      ~band_no, ~band_name,
    ...
    out <- foreach(i = reference_raster) %do% {
      ref <- terra::rast(i)
      ...
      sat_mask <- terra::mask(sat_terra, ref) %>% 
        ...
        terra::writeRaster(filename = file.path(dirname(i), paste0(f_names, ".tif")),
                           overwrite = TRUE)
    } %>% magrittr::set_names(basename(dirname(reference_raster)))
  }))
  return(out)
}
...
for(i in dem_list) {
  if(file.exists(file.path(dirname(i), "sentinel2_mndwi_summer.tif"))) {
    ...
    st_write(
      ...
      dsn = file.path(AOI_dir, "0_raw_inputs", "base_layers", basename(dirname(i)), "water_mndwi.gpkg")
      )
  }
}
