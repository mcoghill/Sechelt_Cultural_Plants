...
dsm_proc <- sapply(map_res_id, function(z) {
  shapes_dir <- file.path(AOI_dir, "0_raw_inputs", "base_layers", z)
  dsmart_dir <- file.path(AOI_dir, "1_map_inputs", "dsmart_2019", z)
  ...
  proc_dir <- file.path(field_data_dir, "processed_2019", z)
  dir.create(proc_dir, showWarnings = FALSE, recursive = TRUE)
  dsmart_site_ser_observations_sf <- dsmart_site_ser_observations %>% 
    st_as_sf(coords = c("X", "Y")) %>% 
    st_set_crs(st_crs(tem_sf)) %>% 
    st_write(file.path(proc_dir, "site_ser_pts.gpkg"), quiet = TRUE, 
             delete_layer = TRUE)
  return(list(
    dsmart_site_ser_polys = dsmart_site_ser_polys,
    dsmart_site_ser_composition = dsmart_site_ser_composition,
    dsmart_site_ser_observations = dsmart_site_ser_observations_sf,
    dsmart_strata = dsmart_strata
  ))
}, simplify = FALSE, USE.NAMES = TRUE)
...
pres_abs_cover <- sapply(map_res_id, function(z) {
  ...
  pto <- file.path(field_data_dir, "processed_2019", z)
  ...
  pres_abs <- sapply(poi, function(x) {
    ...
    out <- bind_rows(sechelt_pres_cov[[x]], tem_xls_pres[[x]], 
                     tem_mdb_pres_cov[[x]]) %>% 
  ...
  cover <- sapply(poi, function(x) {
    out <- bind_rows(sechelt_pres_cov[[x]], tem_mdb_pres_cov[[x]]) %>% 
  ...
  return(list(pres_abs = pres_abs, cover = cover))
}, simplify = FALSE, USE.NAMES = TRUE)
...
str_stg <- sapply(map_res_id, function(z) {
  shapes_dir <- file.path(AOI_dir, "0_raw_inputs", "base_layers", z)
  dsmart_dir <- file.path(AOI_dir, "1_map_inputs", "dsmart_2019", z)
  covariate_dir <- file.path(AOI_dir, "1_map_inputs", "covariates", z)
  ...
  vri_veg_only <- dplyr::select(vri, starts_with(c(
    ...
    "SPECIES_PCT", "PROJ_AGE", "PROJ_HEIGHT", "BEC"))) %>% 
  ...
  vri_avg <- dplyr::mutate(vri_trees,
                           ...
                           ht_avg = ifelse(
                             !is.na(PROJ_HEIGHT_2),
                             ((SPECIES_PCT_1 * PROJ_HEIGHT_1) + (SPECIES_PCT_2 * PROJ_HEIGHT_2)) / 
                               (SPECIES_PCT_1 + SPECIES_PCT_2), 
                             PROJ_HEIGHT_1))
  
  # Add less than 15 year old and less than 10m tall stands to the 3b calls
  ss_3b <- rbind(
    ss_3b,
    dplyr::filter(vri_avg, age_avg < 15, ht_avg < 10) %>% 
      dplyr::select(-c(age_avg, ht_avg)) %>% 
      dplyr::mutate(ss1 = "3b", ss2 = NA, ss3 = NA))
  ...
  ss_4 <- rbind(
    dplyr::filter(vri_avg, age_avg < 15, ht_avg >= 10),
    dplyr::filter(vri_avg, age_avg >= 15, age_avg < 30)) %>% 
    dplyr::mutate(ss1 = "4", ss2 = NA, ss3 = NA)
  ss_5 <- dplyr::filter(vri_avg, age_avg >= 30, age_avg < 80) %>% 
    dplyr::mutate(
      ss1 = "5", 
