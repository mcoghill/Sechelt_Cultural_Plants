...
out_dir <- file.path(AOI_dir, "2_map_predictions")
...
tem_mdb <- access_query_32(db_dir = file.path(field_data_dir, "raw", "tem_data.mdb")) %>% 
  lapply(function(x) {
...
tem_sf <- st_read(file.path(shapes_dir, paste0(map_res, "m"), "tem.gpkg"), quiet = TRUE) %>% 
  select_if(function(x) !all(is.na(x)))
aoi_sf <- st_read(file.path(shapes_dir, paste0(map_res, "m"), "aoi.gpkg"), quiet = TRUE) %>% 
...
# Read in VRI data
vri <- st_read(file.path(shapes_dir, paste0(map_res, "m"), "vri_full.gpkg"), quiet = TRUE)
...
st_write(vri_bgc, file.path(shapes_dir, paste0(map_res, "m"), "bgc_vri.gpkg"), 
         delete_layer = TRUE, quiet = TRUE)
