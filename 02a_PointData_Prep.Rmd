...
ls <- c("plyr", "tidyverse", "readxl", "httr", "svSocket", "R.utils", "RODBC", "sf", "terra")
...
r_32bit_path <- file.path(R.home(),  "bin/i386/Rscript.exe")
...
# nothing wrong with code. You might just need to open this link in your web browser,
# just be sure to save it to the same filder as the TEM spreadsheet
...
tem_xls_bec <- dplyr::select(tem_xls[[1]], Poly_Nbr, ECI_Tag, `BGC Zone`, 
  ...
  mutate(MapUnit = as.factor(make.names(paste0(`BGC Zone`, Subzone, Variant, "/", Sser1)))) %>% 
  st_sf()
...
tem_mdb_bec <- dplyr::select(
  ...
  UTMNorthing, UTMEasting, StructuralStage) %>% 
  dplyr::rename(MapUnit = BECUnit) %>% 
...
dsmart_strata <- group_by(dsmart_polygons, Subzone) %>% 
  dplyr::summarise() %>% 
  mutate(strata = as.integer(Subzone)) %>% 
  st_cast("MULTIPOLYGON")

# Write DSMART outputs
dir.create(file.path(dsmart_dir, "inputs"), recursive = TRUE, showWarnings = FALSE)

st_write(dsmart_polygons, file.path(dsmart_dir, "inputs", "dsmart_polygons.gpkg"), delete_layer = TRUE)
st_write(dsmart_strata, file.path(dsmart_dir, "inputs", "dsmart_strata.gpkg"), delete_layer = TRUE)
...
pres_abs <- sapply(c("RUBUSPE", "CORNCAN", "LEDUGRO", "VERAVIR"), function(x) {
  rbind(sechelt_pres_cov[[x]][, "Pres"], tem_xls_pres[[x]][, "Pres"], 
        tem_mdb_pres_cov[[x]][, "Pres"]) %>% 
...
struct_stage <- dplyr::select(sechelt_bec_full, `Structural Stage`) %>% 
  dplyr::rename(StructuralStage = `Structural Stage`) %>% 
  bind_rows(dplyr::select(tem_mdb_bec, StructuralStage)) %>% 
  mutate(StructuralStage = trimws(StructuralStage)) %>% 
  mutate(StructuralStage = gsub("^$", NA, StructuralStage)) %>% 
  st_transform(crs = st_crs(aoi_sf)) %>% 
  st_intersection(aoi_sf) %>% 
  cbind(terra::extract(covars, st_coordinates(.))) %>% 
  drop_na()
...
dir.create(file.path(field_data_dir, "processed"), recursive = TRUE, showWarnings = FALSE)
...
st_write(struct_stage, file.path(field_data_dir, "processed", "struct_stage.gpkg"), 
         delete_layer = TRUE)

```

Part of the issue with a couple of the metrics is that for a couple of the plant species of interest, there are only a small handful of presence data. This can create issues with overfitting/underfitting a model; however, there are ways around this issue. The RFUtilities package includes a class balance function to balance your sample classes, while native caret has the SMOTE or ROSE algorithm to artificially increase the low class dataset and decrease the high class datasets. Once some maps have been made, it is likely that multiple renditions of these maps will be created in order to see if any of the class balancing acts have any major effects on modelling accuracy.
