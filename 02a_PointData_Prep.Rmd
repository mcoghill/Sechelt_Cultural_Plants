...
dsm_proc <- lapply(map_res, function(z) {
  res_def <- suppressWarnings(as.numeric(z))
  res_dir <- if(!is.na(res_def)) paste0(res_def, "m") else z
...
pres_abs_cover <- lapply(map_res, function(z) {
  res_def <- suppressWarnings(as.numeric(z))
  res_dir <- if(!is.na(res_def)) paste0(res_def, "m") else z
...
str_stg <- lapply(map_res, function(z) {
  res_def <- suppressWarnings(as.numeric(z))
  res_dir <- if(!is.na(res_def)) paste0(res_def, "m") else z
  shapes_dir <- file.path(AOI_dir, "0_raw_inputs", "base_layers", res_dir)
  dsmart_dir <- file.path(AOI_dir, "1_map_inputs", "dsmart_2019", res_dir)
