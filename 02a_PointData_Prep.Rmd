...
tem_mdb_pres_cov <- sapply(c("RUBUSPE", "CORNCAN", "LEDUGRO", "VERAVIR"), function(x) {
  group_by(tem_mdb_merge, PlotID, Species) %>% 
    ...
    summarise(across(Cover, sum), .groups = "drop_last") %>% 
    ungroup() %>% 
...
if(!st_geometry_type(dsmart_polygons, FALSE) %in% c("POLYGON", "MULTIPOLYGON")) {
...
dso <- file.path(dsmart_dir, "inputs")
dir.create(dso, recursive = TRUE, showWarnings = FALSE)

sapply(c("dsmart_polygons", "dsmart_strata"), function(x) st_write(
  eval(parse(text = x)), file.path(dso, paste0(x, ".gpkg")), 
  delete_layer = TRUE, quiet = TRUE))[0]

sapply(c("dsmart_composition", "dsmart_observations"), function(x) write.csv(
...
for(layer in names(pres_abs)) {
  st_write(pres_abs[[layer]], file.path(pto, "pres_abs.gpkg"), 
           layer = layer, delete_layer = TRUE, quiet = TRUE)
  st_write(cover[[layer]], file.path(pto, "cover.gpkg"), 
           layer = layer, delete_layer = TRUE, quiet = TRUE)
}

st_write(struct_stage, file.path(pto, "struct_stage.gpkg"), 
