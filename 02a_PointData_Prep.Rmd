...
```{r Load Packages}
ls <- c("tidyverse", "readxl", "httr", "svSocket", "R.utils", "RODBC", "sf")
...
r_32bit_path <- file.path("C:/Program Files/R/R-3.5.3/bin/i386/Rscript.exe")
```
...
```{r Field data 2019}
sechelt_bec <- read_excel(file.path(field_data_dir, "raw",  "shishalhBECandCulturalPlants_FieldData_2019.xlsx"), sheet = 1) %>% 
  mutate(`Site Disturbance` = gsub("\\s*\\([^\\)]+\\)", "", `Site Disturbance`)) %>% # Remove brakets and everything within them
  mutate(`Site Disturbance` = substr(`Site Disturbance`, start = 1, stop = 5)) %>% # Shorten strings to 5 characters max
  mutate(`Site Disturbance` = gsub("^[[:lower:]]+", NA, `Site Disturbance`)) %>% # Remove strings starting with lowercase
  mutate(`Site Disturbance` = gsub(", +", ",", `Site Disturbance`)) %>% # In strings with commas, remove white space
  mutate(`Site Disturbance` = sub(" .*", "", `Site Disturbance`)) %>% # In strings with white space, remove the text after the white space
  mutate(`Site Disturbance` = gsub(",[a-z]+", "", `Site Disturbance`)) %>%  # If there is a comma, keep only text after it if it does not begin with lowercase
  mutate(`Site Disturbance` = str_replace_all(`Site Disturbance`, "[^[A-Za-z,]]", " ") %>% str_squish(.)  %>% str_replace_all(.," , ", ", ")) %>% # Removes all special characters except for commas
  mutate(`Site Disturbance` = gsub(",$", "", `Site Disturbance`)) %>% # Removes trailing commas
...
sechelt_rubuspe <- read_excel(file.path(field_data_dir, "raw", "shishalhBECandCulturalPlants_FieldData_2019.xlsx"), sheet = 2)
sechelt_veravir <- read_excel(file.path(field_data_dir, "raw", "shishalhBECandCulturalPlants_FieldData_2019.xlsx"), sheet = 3)
sechelt_corncan <- read_excel(file.path(field_data_dir, "raw", "shishalhBECandCulturalPlants_FieldData_2019.xlsx"), sheet = 4)

# Using tidyverse to tidy up the sechelt_bec table
# 1. Make site series calls pure calls only (for simplicity)
# 2. Apply fix to incorrectly labelled BGC subzones
# 3. Create map unit column
# 4. Make an sf dataframe using lat/long and transform to EPSG 3005
sechelt_bec_sf <- dplyr::filter(sechelt_bec, nchar(`Forested Site Series`) == 2) %>% 
  mutate(`BGC Subzone/variant` = str_replace(`BGC Subzone/variant`, "CHWxm", "CWHxm"), 
         `BGC Subzone/variant` = str_replace(`BGC Subzone/variant`, "CWHxm", "CWHxm1"),
         MapUnit = as.factor(make.names(paste0(`BGC Subzone/variant`, "/", `Forested Site Series`)))) %>% 
  st_as_sf(coords = c("Long", "Lat"), crs = 4326) %>% 
  st_transform(3005)

# The species tables don't have coordinates associated with them, but they can be merged with the BEC table which has that data
sechelt_rubuspe_sf <- dplyr::select(sechelt_bec, `Point ID`, Long, Lat, `Structural Stage`, `Site Disturbance`, `% Canopy Cover`) %>% 
  merge(sechelt_rubuspe) %>% 
  mutate(Cover = rowSums(.[c("A1", "A2", "A3", "B1", "B2", "C", "D")], na.rm = TRUE)) %>% 
  mutate(RUBUSPE_Pres = Cover > 0, 
         `Structural Stage` = as.factor(`Structural Stage`), 
         `Site Disturbance` = as.factor(`Site Disturbance`)) %>% 
  st_as_sf(coords = c("Long", "Lat"), crs = 4326) %>% 
  st_transform(3005) %>% 
  dplyr::select(`Point ID`, Cover, RUBUSPE_Pres, `Structural Stage`, `Site Disturbance`, `% Canopy Cover`) %>% 
  drop_na()
  
sechelt_veravir_sf <- dplyr::select(sechelt_bec, `Point ID`, Long, Lat,`Structural Stage`, `Site Disturbance`, `% Canopy Cover`) %>% 
  merge(sechelt_veravir) %>% 
  mutate(Cover = rowSums(.[c("A1", "A2", "A3", "B1", "B2", "C", "D")], na.rm = TRUE)) %>% 
  mutate(VERAVIR_Pres = Cover > 0, 
         `Structural Stage` = as.factor(`Structural Stage`), 
         `Site Disturbance` = as.factor(`Site Disturbance`)) %>% 
  st_as_sf(coords = c("Long", "Lat"), crs = 4326) %>% 
  st_transform(3005) %>% 
  dplyr::select(`Point ID`, Cover, VERAVIR_Pres, `Structural Stage`, `Site Disturbance`, `% Canopy Cover`) %>% 
  drop_na()

sechelt_corncan_sf <- dplyr::select(sechelt_bec, `Point ID`, Long, Lat, `Structural Stage`, `Site Disturbance`, `% Canopy Cover`) %>% 
  merge(sechelt_corncan) %>% 
  dplyr::filter(Comments != "*not recorded" | is.na(Comments)) %>% 
  mutate(Cover = rowSums(.[c("A1", "A2", "A3", "B1", "B2", "C", "D")], na.rm = TRUE)) %>% 
  mutate(CORNCAN_Pres = Cover > 0, 
         `Structural Stage` = as.factor(`Structural Stage`), 
         `Site Disturbance` = as.factor(`Site Disturbance`)) %>% 
  st_as_sf(coords = c("Long", "Lat"), crs = 4326) %>% 
  st_transform(3005) %>% 
  dplyr::select(`Point ID`, Cover, CORNCAN_Pres, `Structural Stage`, `Site Disturbance`, `% Canopy Cover`) %>% 
  drop_na()
...
sechelt_ledugro <- read_excel(file.path(field_data_dir, "raw", "shishalhBECandCulturalPlants_FieldData_2019.xlsx"), sheet = 5)
sechelt_ledugro_sf <- dplyr::select(sechelt_bec, `Point ID`, Long, Lat,`Structural Stage`, `Site Disturbance`, `% Canopy Cover`) %>% 
  merge(sechelt_ledugro) %>% 
  dplyr::filter(Comments != "*not recorded" | is.na(Comments)) %>% 
  mutate(B2 = as.numeric(unlist(str_extract_all(B2,"\\(?[0-9,.]+\\)?")))) %>% 
  mutate(Cover = ifelse(Species == "Ledgro", rowSums(.[c("A1", "A2", "A3", "B1", "B2", "C", "D")], na.rm = TRUE), 0)) %>% 
  mutate(LEDUGRO_Pres = Cover > 0, 
         `Structural Stage` = as.factor(`Structural Stage`), 
         `Site Disturbance` = as.factor(`Site Disturbance`)) %>% 
  st_as_sf(coords = c("Long", "Lat"), crs = 4326) %>% 
  st_transform(3005) %>% 
  dplyr::select(`Point ID`, Cover, LEDUGRO_Pres,`Structural Stage`, `Site Disturbance`, `% Canopy Cover`) %>% 
  drop_na()

dplyr::select(sechelt_corncan_sf, -c(`Point ID`, Cover)) %>% 
  st_write(file.path(AOI_dir, "1_map_inputs", "field_data", "processed", "corncan_extravars_PresAbs.gpkg"), delete_dsn = TRUE, delete_layer = TRUE)

dplyr::select(sechelt_ledugro_sf, -c(`Point ID`, Cover)) %>% 
  st_write(file.path(AOI_dir, "1_map_inputs", "field_data", "processed", "ledugro_extravars_PresAbs.gpkg"), delete_dsn = TRUE, delete_layer = TRUE)

dplyr::select(sechelt_rubuspe_sf, -c(`Point ID`, Cover)) %>% 
  st_write(file.path(AOI_dir, "1_map_inputs", "field_data", "processed", "rubuspe_extravars_PresAbs.gpkg"), delete_dsn = TRUE, delete_layer = TRUE)

dplyr::select(sechelt_veravir_sf, -c(`Point ID`, Cover)) %>% 
st_write(file.path(AOI_dir, "1_map_inputs", "field_data", "processed", "veravir_extravars_PresAbs.gpkg"), delete_dsn = TRUE, delete_layer = TRUE)
...
tem_data <- GET("http://a100.gov.bc.ca/appsdata/acat/documents/r35895/tem_4678_eci_1363698000276_6de51d6c988801e3369406fe9a22713aa4dca27f63de77d54bf44b70a246813b.xls", write_disk(file.path(field_data_dir, "raw", "tem_data.xls"), overwrite = TRUE))

tem_xls <- read_xls(file.path(field_data_dir, "raw", "tem_data.xls"), skip = 1) %>% 
  mutate(RUBUSPE_Pres = grepl("RUBUSPE", Comments),
         CORNCAN_Pres = grepl("CORNCAN", Comments),
         VERAVIR_Pres = grepl("VERAVIR", Comments),
         LEDUGRO_Pres = grepl("LEDUGRO", Comments))

tem_xls_sf <- st_as_sf(tem_xls, coords = c("UTM_East", "UTM_North"), crs = 26910) %>% 
  st_transform(3005)

tem_mdb <- GET("http://a100.gov.bc.ca/appsdata/acat/documents/r35895/tem_4678_eci_1363697969131_6de51d6c988801e3369406fe9a22713aa4dca27f63de77d54bf44b70a246813b.mdb", write_disk(file.path(field_data_dir, "raw", "tem_data.mdb"), overwrite = TRUE))

access_query_32(db_path = file.path(field_data_dir, "raw", "tem_data.mdb")) 

# Extract all of the information from the database
for(k in 1:length(TableList)){
  l <- TableList[[k]]
  do.call("<-", list(names(TableList[k]), TableList[[k]])) # Separate the tables into the R environment
}

# Only keep the tables with data in them (delete tables with no data)
mdb_objects <- objects(pattern = "MDB_")
for(k in mdb_objects) {
  table <- get(k)
  if(class(table) == "data.frame") {
    if(nrow(table) == 0) {
      rm(list = c(objects(pattern = paste0("^", k, "$")), "table"))
    }
  } else {
    rm(list = c(objects(pattern = paste0("^", k, "$")), "table"))
  }
...
# Clean up the remaining tables (keeping only the ones with useful information)
rm(`MDB_Object Definition`, MDB_PreVegQry, MDB_qryVegTreeCross, MDB_tblProjectManager, MDB_tblValidation, MDB_tmpObjectDefinition, MDB_USysVersion)

# Should be left with 3 tables containing the relevant data: PlotData, VegLayer, and VegSpecies
...
```{r TEM processing}
tem_sf <- st_read(file.path(shapes_path, "4m", "tem.gpkg"))

tem_map_info <- dplyr::select(
...
tem_map_info$geom[[474]] <- tem_map_info$geom[[474]][[2]] # Removes a point feature causing issues
tem_map_info <- st_cast(tem_map_info, "MULTIPOLYGON")


# From the TEM data, create a subzone layer which will be used to stratify sampling
dsmart_strata_sf <- ungroup(tem_map_info) %>% 
  group_by(Subzone) %>% 
  dplyr::summarise() %>% 
  mutate(strata = as.numeric(Subzone)) %>% 
  st_cast("MULTIPOLYGON")
st_write(dsmart_strata_sf, file.path(dsmart_dir, "inputs", "dsmart_strata.gpkg"), delete_dsn = TRUE)

# Convert polygon composition to table with 5 columns
tem_map_composition <- tem_map_info %>% 
  st_drop_geometry() %>% 
  #merge(level_table) %>% 
...
```{r dsmart data prep}
# TEM excel spreadsheet - BEC info
tem_xls_bec_data <- dplyr::select(tem_xls_sf, Poly_Nbr, ECI_Tag, `BGC Zone`, Subzone, Variant, Sser1, Sser2, Sser3, SDec1, Sdec2, Sdec3) %>% 
  ...
  mutate(MapUnit = ifelse(grepl("^[A-Za-z]+$", Sser1), Sser1, paste0(`BGC Zone`, Subzone, Variant, "/", Sser1))) %>% 
  mutate(MapUnit = as.factor(make.names(MapUnit))) %>% 
  st_sf() %>% 
  dplyr::select(MapUnit)

# MDB_tblPlotData has way too many columns, keep only important ones for BEC
tem_mdb_bec_data <- dplyr::select(MDB_tblPlotData, PlotID, PlotNumber, BECUnit, SiteSeries, SiteSeriesSymbol, UTMNorthing, UTMEasting) %>% 
  rename(MapUnit = BECUnit) %>% 
  mutate(MapUnit = ifelse(SiteSeries == 0, as.character(SiteSeriesSymbol), as.character(MapUnit))) %>% 
  mutate(MapUnit = as.factor(make.names(MapUnit))) %>% 
  st_as_sf(coords = c("UTMEasting", "UTMNorthing"), crs = 26910) %>% 
  st_transform(3005) %>% 
  dplyr::select(MapUnit)

sechelt_bec_data <- dplyr::select(sechelt_bec_sf, MapUnit)

all_bec_info <- rbind(sechelt_bec_data, tem_xls_bec_data, tem_mdb_bec_data) %>% 
  st_intersection(tem_sf) %>% 
  dplyr::select(MapUnit) %>% 
  droplevels()

dsmart_observations <- cbind(as.data.frame(st_coordinates(all_bec_info)), 
                             data.frame(MapUnit = make.names(all_bec_info$MapUnit)))
dsmart_polygons <- as(tem_map_info, "Spatial")
dsmart_composition <- as.data.frame(tem_map_composition)

ifelse(!dir.exists(file.path(dsmart_dir, "inputs")), dir.create(file.path(dsmart_dir, "inputs")), FALSE)
...
writeOGR(dsmart_polygons, file.path(dsmart_dir, "inputs", "dsmart_polygons.gpkg"), layer = "dsmart_polygons", driver = "GPKG", overwrite_layer = TRUE, delete_dsn = TRUE)
```



```{r Presence Absence Info}
sechelt_rubu_pres <- dplyr::select(sechelt_rubuspe_sf, RUBUSPE_Pres)
sechelt_corn_pres <- dplyr::select(sechelt_corncan_sf, CORNCAN_Pres)
sechelt_vera_pres <- dplyr::select(sechelt_veravir_sf, VERAVIR_Pres)
sechelt_ledu_pres <- dplyr::select(sechelt_ledugro_sf, LEDUGRO_Pres)

tem_xls_rubu_pres <- dplyr::select(tem_xls_sf, RUBUSPE_Pres)
tem_xls_corn_pres <- dplyr::select(tem_xls_sf, CORNCAN_Pres)
tem_xls_vera_pres <- dplyr::select(tem_xls_sf, VERAVIR_Pres)
tem_xls_ledu_pres <- dplyr::select(tem_xls_sf, LEDUGRO_Pres)

tem_mdb_species <- merge(MDB_tblVegLayer, MDB_tblVegSpecies) %>% 
  merge(MDB_tblPlotData) %>% 
  st_as_sf(coords = c("UTMEasting", "UTMNorthing"), crs = 26910) %>% 
  st_transform(3005) %>% 
  group_by(PlotID, Species) %>% 
  dplyr::filter(Species %in% c("RUBUSPE", "CORNCAN", "LEDUGRO")) %>% 
  summarise_at(vars(Cover), sum) %>% 
  mutate(RUBUSPE_Pres = Species == "RUBUSPE",
         CORNCAN_Pres = Species == "CORNCAN",
         LEDUGRO_Pres = Species == "LEDUGRO") %>% 
  dplyr::select(PlotID, Species, RUBUSPE_Pres, CORNCAN_Pres, LEDUGRO_Pres, Cover) %>% 
  ungroup()

tem_mdb_rubu_pres <- dplyr::filter(MDB_tblPlotData, PlotID %in% MDB_tblPlotData$PlotID[!MDB_tblPlotData$PlotID %in% tem_mdb_species$PlotID[tem_mdb_species$Species == "RUBUSPE"]]) %>% 
  st_as_sf(coords = c("UTMEasting", "UTMNorthing"), crs = 26910) %>% 
  st_transform(3005) %>% 
  mutate(RUBUSPE_Pres = FALSE) %>% 
  dplyr::select(RUBUSPE_Pres) %>% 
  rbind(tem_mdb_species[tem_mdb_species$Species == "RUBUSPE", "RUBUSPE_Pres"])

tem_mdb_corn_pres <- dplyr::filter(MDB_tblPlotData, PlotID %in% MDB_tblPlotData$PlotID[!MDB_tblPlotData$PlotID %in% tem_mdb_species$PlotID[tem_mdb_species$Species == "CORNCAN"]]) %>% 
  st_as_sf(coords = c("UTMEasting", "UTMNorthing"), crs = 26910) %>% 
  st_transform(3005) %>% 
  mutate(CORNCAN_Pres = FALSE) %>% 
  dplyr::select(CORNCAN_Pres) %>% 
  rbind(tem_mdb_species[tem_mdb_species$Species == "CORNCAN", "CORNCAN_Pres"])

tem_mdb_ledu_pres <- dplyr::filter(MDB_tblPlotData, PlotID %in% MDB_tblPlotData$PlotID[!MDB_tblPlotData$PlotID %in% tem_mdb_species$PlotID[tem_mdb_species$Species == "LEDUGRO"]]) %>% 
  st_as_sf(coords = c("UTMEasting", "UTMNorthing"), crs = 26910) %>% 
  st_transform(3005) %>% 
  mutate(LEDUGRO_Pres = FALSE) %>% 
  dplyr::select(LEDUGRO_Pres) %>% 
  rbind(tem_mdb_species[tem_mdb_species$Species == "LEDUGRO", "LEDUGRO_Pres"])

# Combine them all
rubuspe_pres_abs <- rbind(sechelt_rubu_pres, tem_mdb_rubu_pres, tem_xls_rubu_pres) %>% 
  st_intersection(tem_sf) %>% 
  dplyr::select(RUBUSPE_Pres)
corncan_pres_abs <- rbind(sechelt_corn_pres, tem_mdb_corn_pres, tem_xls_corn_pres) %>% 
  st_intersection(tem_sf) %>% 
  dplyr::select(CORNCAN_Pres)
ledugro_pres_abs <- rbind(sechelt_ledu_pres, tem_mdb_ledu_pres, tem_xls_ledu_pres) %>% 
  st_intersection(tem_sf) %>% 
  dplyr::select(LEDUGRO_Pres)
veravir_pres_abs <- rbind(sechelt_vera_pres, tem_xls_vera_pres) %>% 
  st_intersection(tem_sf) %>% 
  dplyr::select(VERAVIR_Pres)

ifelse(!dir.exists(file.path(field_data_dir, "processed")), dir.create(file.path(field_data_dir, "processed"), recursive = TRUE))

st_write(rubuspe_pres_abs, file.path(field_data_dir, "processed", "rubuspe_PresAbs.gpkg"), delete_dsn = TRUE)
st_write(corncan_pres_abs, file.path(field_data_dir, "processed", "corncan_PresAbs.gpkg"), delete_dsn = TRUE)
st_write(veravir_pres_abs, file.path(field_data_dir, "processed", "veravir_PresAbs.gpkg"), delete_dsn = TRUE)
st_write(ledugro_pres_abs, file.path(field_data_dir, "processed", "ledugro_PresAbs.gpkg"), delete_dsn = TRUE)
...
```{r Cover info}
tem_mdb_rubu_cov <- dplyr::filter(MDB_tblPlotData, PlotID %in% MDB_tblPlotData$PlotID[!MDB_tblPlotData$PlotID %in% tem_mdb_species$PlotID[tem_mdb_species$Species == "RUBUSPE"]]) %>% 
  st_as_sf(coords = c("UTMEasting", "UTMNorthing"), crs = 26910) %>% 
  st_transform(3005) %>% 
  mutate(Cover = 0) %>% 
  dplyr::select(Cover) %>% 
  rbind(tem_mdb_species[tem_mdb_species$Species == "RUBUSPE", "Cover"])

tem_mdb_corn_cov <- dplyr::filter(MDB_tblPlotData, PlotID %in% MDB_tblPlotData$PlotID[!MDB_tblPlotData$PlotID %in% tem_mdb_species$PlotID[tem_mdb_species$Species == "CORNCAN"]]) %>% 
  st_as_sf(coords = c("UTMEasting", "UTMNorthing"), crs = 26910) %>% 
  st_transform(3005) %>% 
  mutate(Cover = 0) %>% 
  dplyr::select(Cover) %>% 
  rbind(tem_mdb_species[tem_mdb_species$Species == "CORNCAN", "Cover"])

tem_mdb_ledu_cov <- dplyr::filter(MDB_tblPlotData, PlotID %in% MDB_tblPlotData$PlotID[!MDB_tblPlotData$PlotID %in% tem_mdb_species$PlotID[tem_mdb_species$Species == "LEDUGRO"]]) %>% 
  st_as_sf(coords = c("UTMEasting", "UTMNorthing"), crs = 26910) %>% 
  st_transform(3005) %>% 
  mutate(Cover = 0) %>% 
  dplyr::select(Cover) %>% 
  rbind(tem_mdb_species[tem_mdb_species$Species == "LEDUGRO", "Cover"])

rubuspe_cov_data <- dplyr::select(sechelt_rubuspe_sf, Cover) %>% rbind(tem_mdb_rubu_cov) %>% 
  st_intersection(tem_sf) %>% 
  dplyr::select(Cover) 
corncan_cov_data <- dplyr::select(sechelt_corncan_sf, Cover) %>% rbind(tem_mdb_corn_cov) %>% 
  st_intersection(tem_sf) %>% 
  dplyr::select(Cover)
ledugro_cov_data <- dplyr::select(sechelt_ledugro_sf, Cover) %>% rbind(tem_mdb_ledu_cov) %>% 
  st_intersection(tem_sf) %>% 
  dplyr::select(Cover)
veravir_cov_data <- dplyr::select(sechelt_veravir_sf, Cover) %>% 
  st_intersection(tem_sf) %>% 
  dplyr::select(Cover)

ifelse(!dir.exists(file.path(field_data_dir, "processed")), dir.create(file.path(field_data_dir, "processed"), recursive = TRUE))

st_write(rubuspe_cov_data, file.path(field_data_dir, "processed", "rubuspe_cover.gpkg"), delete_dsn = TRUE)
st_write(corncan_cov_data, file.path(field_data_dir, "processed", "corncan_cover.gpkg"), delete_dsn = TRUE)
st_write(ledugro_cov_data, file.path(field_data_dir, "processed", "veravir_cover.gpkg"), delete_dsn = TRUE)
st_write(veravir_cov_data, file.path(field_data_dir, "processed", "ledugro_cover.gpkg"), delete_dsn = TRUE)
```
