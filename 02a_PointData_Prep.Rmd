...
sechelt_plants <- read_excel(spreadsheet, sheet = "sechelt_plants") %>% 
  ...
  left_join(sechelt_bec, by = c("point_id" = "Point.ID")) %>% 
  dplyr::select(point_id, Species, Lat, Long, Pres, Cover) %>% 
...
tem_mdb_pres_cov <- sapply(poi, function(x)
  dplyr::filter(tem_mdb_merge, Species == x) %>% 
    dplyr::select(point_id, Pres, Cover), simplify = FALSE, USE.NAMES = TRUE)
...
dsmart_site_ser_observations <- lapply(
  list(sechelt_bec, tem_xls_bec, tem_mdb_bec), function(x)
  ...
  cbind(
    foreach(i = 1:nrow(.), .combine = rbind) %do% {
      st_distance(.[i, ], dsmart_strata[dsmart_strata$Subzone == .[i, ]$subzone, ]) %>% 
        as.data.frame() %>% 
        magrittr::set_names("dist")
    }) %>% 
  dplyr::filter(dist <= units::as_units(100, "m")) %>%
...
sapply(
  c("dsmart_site_ser_composition", "dsmart_site_ser_observations"), function(x) 
    write.csv(
      eval(parse(text = x)), 
      file.path(dso, paste0(x, ".csv")), row.names = FALSE))[0]

# Write output as processed points in separate folder
proc_dir <- file.path(field_data_dir, res_dir, "processed_2019")
...
cover <- sapply(poi, function(x) {
  ...
  out_att <- cbind(out, terra::extract(covariates, st_coordinates(out))) %>% 
    Filter(function(y) !all(is.na(y)), .) %>% 
    dplyr::select(Cover, names(covariates)) %>% 
    drop_na()
...
dsmart_struct_stg_polys <- rbind(
  ...
  dplyr::mutate(
    ...
    across(ss1, ~ as.factor(make.names(.))),
    across(c(ss1, ss2), ~ as.factor(sub(".*NA\\.", NA, make.names(.)))),
...
sapply(c("dsmart_struct_stg_composition", "dsmart_struct_stg_observations"), 
       function(x) 
         write.csv(
