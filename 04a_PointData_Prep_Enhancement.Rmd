...
plants_2020_fix <- do.call(rbind, lapply(unique(bgc_2020$point_id), function(i) {
  site <- plants_2020 %>% 
    dplyr::filter(point_id == i) %>% 
    as.data.frame()
  if(nrow(site) == 0) {
    site[1, c("point_id", "Lat", "Long")] <- c(
      i, bgc_2020[bgc_2020$point_id == i, ]$Lat,
      bgc_2020[bgc_2020$point_id == i, ]$Long)
  }
...
}) %>% stats::setNames(c("Rubu spe", "Corn can", "Vera vir", "Ledu gro"))
...
pres_abs_2020_focal <- sapply(c("Rubu spe", "Corn can"), function(x) {
  
  # Create unattributed points and write those
  st_write(points_comb[[x]], paste0(x, "_pres_abs_raw.shp"), delete_layer = TRUE,
           quiet = TRUE)
