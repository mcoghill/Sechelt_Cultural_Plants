...
suppressMessages(suppressWarnings({
  ls <- c("plyr", "tidyverse", "readxl", "foreach", "sf", "terra")
...
plants_2020_fix <- foreach(i = unique(plants_2020$`Point ID`), .combine = rbind) %do% {
  site <- plants_2020 %>% 
    ...
    as.data.frame()
  if(nrow(site) == 0) {
    site[1, c("Point ID", "Lat", "Long")] <- c(i, bgc_2020[bgc_2020$`Point ID` == i, ]$Lat,
...
pres_abs_2020 <- sapply(names(points_comb), function(x) {
  ...
  out_att <- cbind(points_comb[[x]], terra::extract(covariates, st_coordinates(points_comb[[x]]))) %>% 
...
struct_stg_old <- st_read(file.path(old_pto, "struct_stg_pts.gpkg"), quiet = TRUE) %>% 
