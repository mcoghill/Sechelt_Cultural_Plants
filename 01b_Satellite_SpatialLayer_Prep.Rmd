...
ls <- c("sf", "raster", "mapview", "rgdal", "tools", "xml2", "RStoolbox", "reticulate", "googledrive", "googleCloudStorageR", "stars", "tidyverse", "stringi")
github.packages <- c("getSpatialData", "rgee")
new.packages <- ls[!(ls %in% installed.packages()[, "Package"])]
new.github.packages <- github.packages[!github.packages %in% installed.packages()[, "Package"]]
if(length(new.packages))
  install.packages(new.packages)
if("getSpatialData" %in% new.github.packages)
  devtools::install_github(c("16EAGLE/getSpatialData"))
if("rgee" %in% new.github.packages)
  remotes::install_github("csaybar/rgee")
ls <- c(ls, github.packages)
lapply(ls, library, character.only = TRUE)
rm(ls, github.packages, new.packages, new.github.packages)
first_conda_install <- FALSE
```

The following chunk should only need to be ran once. This is where I anticipate some problems to arise. The reticulate and rgee packages rely on using the conda distribution of python in order to communicate with the google earth engine. The steps that need to be followed are simple but I could see some pitfalls which I will outline here:

1. You have to have conda installed. This may already be done on a given system, however if it is not then it will be installed. If it is already installed, I'm not sure what that effect is with the given code below. It may require that a user uninstalls it from their system in order to run this properly.

2. After a successful installation, a new python environment needs to be created (ee_create_pyenv). I ran into some trouble with this on my system; if you have multiple installations of python on your system then you may run into some issues with how R is interpreting the python installation from the R environment (use py_config() to see the note on how the python version was forced and Sys.getenv() to see if the call to python is already bound somewhere else; if so, then that environment will probably need to be unset before continuing, though this isn't extensively tested).

3. After successful environment creating, ee_set_pyenv will setup the python environment for google earth engine processing. After this is done, R needs to be restarted in order to see the changes (.rs.restartR()), though in this type of R restart all of the packages and previous objects in your R environment are reloaded. 

4. After reloading the R environment, python packages will need to be installed to the python environment you set up in step 3. This automatically downloads and installs those packages, however R needs to be restarted again in order to see the changes.

Once this is done, it shouldn't need to be ran again. If you run into issues with your current python environment you can make a new environment by setting the env variable and rerunning all code pieces below that.

```{r reticulate package setup}
if(first_conda_install == TRUE) {
  try(install_miniconda())
  env <- "sentinel"
  ee_create_pyenv(env)
  
  # Set a Python environment and restart R to see changes
  ee_set_pyenv(python_path = file.path(miniconda_path(), "envs", env, "python.exe"), 
               python_env = env,
               install = TRUE, 
               confirm = FALSE)
  .rs.restartR()
  
  # Install python packages and restart R to see changes
...
AOI <- file.path(AOI_dir)
AOI_epsg <- 3005
# Define existing files and paths
AOI_shapefile <- file.path(AOI, "0_raw_inputs", "base_layers", "4m", "aoi.gpkg")
dem_files <- list.files(file.path(AOI, "1_map_inputs/covariates"), 
                        pattern = "dem.tif", full.names = TRUE, recursive = TRUE)
# Define path for file processing (doesn't have to exist)
raw_path <- file.path(AOI, "0_raw_inputs", "satellite_layers")
# Define path for satellite index outputs
output_path <- file.path(AOI, "1_map_inputs", "covariates")
# Define path for SAGA GIS installation
# Please note that SAGA version 7.5.0 has a grid transformation bug, please
# use a different version.
# Additionally, download and installation of SAGA GIS may be scripted
...
# This is a rewrite of the original function satellite_download used to download and process a single sentinel image scene. It is no longer recommended to use this function.
get_sentinel_scene <- function(
  ...
  if(is.null(satellite_query_table) || !is.data.frame(satellite_query_table)) {
    stop("You must supply a data.frame object created from the getSentinel_query function")
  }
  
  if(is.null(satellite_record_ids) || !is.numeric(satellite_record_ids)) {
    stop("satellite_record_ids must be a numeric vector corresponding to the row ids of your query table")
  }
  ...
  sentinel_record_date <- substr(satellite_query_table$beginposition[satellite_record_ids[1]], 
                                 start = 1, stop = 10)
  download_path <- file.path(download_folder, sentinel_record_date)
  ifelse(!dir.exists(download_path), dir.create(download_path, recursive = TRUE), FALSE)
  
  # Download data
  satellite_datasets <- getSentinel_data(records = satellite_query_table[satellite_record_ids, ], 
  ...
  for(i in satellite_datasets) {
    ...
    # Find the unique band names only (no duplicate bands at different resolutions)
    sen_band_info <- unique(gsub(".{4}$", "", gsub(".*m/|.jp2$", "", sen_files_zip))) 
    sen_file_list <- NULL
    for(j in sen_band_info){ 
      # Make a list of the bands to extract using the highest resolution possible for each band
      if(paste0(j, "_10m.jp2") %in% basename(sen_files_zip)){
        sen_file_list <- c(sen_file_list, 
    ...
    sen_files <- unzip(i, files = sen_file_list, junkpaths = TRUE, exdir = extract_folder)
  }
  satellite_files <- list.files(extract_folder, pattern = ".jp2$", full.names = TRUE)
  ...
  if(length(satellite_datasets) > 1) {
    ...
    for(i in unique_band_id) {
      ...
      # SAGA is much faster at computing mosaic overlays than the raster package in R
      sys_cmd <- paste(saga_cmd, "grid_tools 3", 
...
# This is a new function, allowing users to process Sentinel 2 bands in an area over 
# the course of a given time frame. It harnesses the computational power of Google
# Earth Engine, making the computations much faster than it would be normally. This is
# now the recommended function.
get_sentinel_ee <- function(
  ...
  if(!any(class(aoi) %in% c("sf", "sfc")) || is.null(aoi)) {
    stop("You must specify an AOI as an sf or sfc object")
  } else if(any(class(aoi) %in% "sf")) {
    ee_geom <- st_geometry(aoi) %>% sf_as_ee()
  } else if(any(class(aoi) %in% "sfc")) {
    ee_geom <- sf_as_ee(aoi)
  }
  
  if(is.null(template_raster) || !class(template_raster) %in% c("RasterLayer", "character")) {
    stop("You must define the template_raster variable as either a 'RasterLayer'
          \nobject or a character string file path to a raster image")
  ...
  if(is.null(date_ranges) || !class(date_ranges) %in% "data.frame") {
    stop("You must supply a data.frame with three columns: 
         \n1: An ID column; 
         \n2: A column of start dates; and 
         \n3: A column of end dates. 
         \nDates must be supplied as a character vector with the format 'YYYY-MM-DD'")
  }
  ...
  if(is.null(download_folder) || !class(download_folder) %in% "character") {
    stop("You must supply an output directory for the files to be downloaded to")
  }
  ...
  cloud_mask <- function(image) {
    ...
    cloudBitMask <- 1024L
    cirrusBitMask <- 2048L
    mask <- qa$bitwiseAnd(cloudBitMask)$eq(0L)$
      And(qa$bitwiseAnd(cirrusBitMask)$eq(0L))
  ...
  bands <- data.frame(
    band = c("B1", "B2", "B3", "B4", "B5", "B6", "B7", "B8", "B9", "B11", "B12", "B8A"), 
    new_name = c("b01", "b02", "b03", "b04", "b05", "b06", "b07", "b08", "b09", "b11", "b12", "b8A"),
    res = c(60, 10, 10, 10, 20, 20, 20, 10, 60, 20, 20, 20), stringsAsFactors = FALSE)
  
  for(i in 1:nrow(date_ranges)) {
    date_id <- date_ranges[i, 1]
    message(paste0("\nStarting processing of ", date_id, " dates\n"))
    ...
    for(j in 1:nrow(bands)) {
      task_img <- ee$batch$Export$image$toDrive(
        image = comp_clip$select(list(bands$band[j]))$reproject("EPSG:3005", NULL, bands$res[j]),
        folder = drive_get(paste0("Sentinel2/", date_id))$name,
        maxPixels = ncell(template_raster)*2, 
      ...
      img <- ee_download_drive(task_img, 
                               filename = file.path(out_dir, paste0(bands$new_name[j])), 
                               overwrite = TRUE)
    }
  }
...
```{r Process single Sentinel 2 scene}
...
```{r Process composite images in GEE}
ee_Initialize(drive = TRUE)
AOI_shape <- st_read(AOI_shapefile) %>% 
  st_buffer(dist = 250) %>% 
...
seasons_df <- data.frame(
  ...
  end = c("2019-03-19", "2019-06-20", "2019-09-22", "2019-12-20"), stringsAsFactors = FALSE)
ee_download <- get_sentinel_ee(
  aoi = AOI_shape, 
  template_raster = dem_files[1], 
...
bands_to_indices <- function(
  ...
  process_path = NULL, 
  save = TRUE, 
  covariate_file_path = NULL, 
  remove_index = NULL) 
  {
  
  # Perform input checks
  if(is.null(reference_raster) || !class(reference_raster) %in% c("character", "RasterLayer")) {
    stop("You must specify a file path to a raster image")
  } else if(class(reference_raster) == "RasterLayer") {
    reference_raster <- reference_raster@file@name
  } else if(class(reference_raster) == "character") {
  ...
  if(is.null(process_path) || !is.character(process_path)) {
    process_path <- file.path(paste0(dirname(satellite_files[1]), "_reproject"))
  }
  
  if((save == TRUE && is.null(covariate_file_path)) || 
     (save == TRUE && !is.character(covariate_file_path))) {
    stop("You must specify a path to save the ouput files to when save = TRUE")
  }
  
  ifelse(!dir.exists(process_path), dir.create(process_path, recursive = TRUE), FALSE)
  
  # Reproject and mask the band layers
  raster_res <- res(raster(reference_raster))[1]
  satellite_out <- file.path(process_path, paste0(raster_res, "m"))
  ifelse(!dir.exists(satellite_out), dir.create(satellite_out, recursive = TRUE), FALSE)
  mask_xml <- c()
  
  # Create the SAGA toolchain for bulk grid transformation
  
  
  # Add to the toolchain by looping through the files since they require masking individually
  for(j in 1:length(satellite_files)){
    satellite_proj <- st_crs(raster(satellite_files[j]))$proj4string
    band_num <- gsub("^[aA-zZ]", "", basename(file_path_sans_ext(satellite_files[j]))) %>% 
      stri_pad_left(2, 0)
    
    k <- paste0("sentinel2_b", band_num)
    ...
    mask_xml <- paste(mask_xml, paste0("
      ...
      <tool library='grid_tools' tool='24' name='Grid Masking'>
        <input id='GRID'>", k, "</input>
        <input id='MASK'>DEM_REFERENCE</input>
        <output id='MASKED'>", paste0(k, " [masked]"), "</output>
      </tool>
      <tool library='io_gdal' tool='2' name='Export GeoTIFF'>
            <input id='GRIDS'>", paste0(k, " [masked]"), "</input>
            <option id='FILE'>", normalizePath(file.path(satellite_out, paste0(k, ".tif"))), "</option>
      </tool>"), collapse = ' ')
  }
  
  satellite_processing_xml <- paste(deparse(paste0(
    "<?xml version='1.0' encoding='UTF-8'?>
    <toolchain saga-version='7.4.0'>
    <group>toolchains</group>
    <identifier>Satellite</identifier>
    <name>Satellilte_Processing</name>
    ...
    <tools>
      <tool library='pj_proj4' tool='0' name='Set Coordinate Reference System'>
        <option id='CRS_PROJ4'>", satellite_proj, "</option>
  ...
  # Write out the toolchain
  write_xml(read_xml(eval(parse(text = satellite_processing_xml))), 
            file.path(dirname(saga_path), "tools", "toolchains", "PEM_Satellite_Processing.xml"))
  
  # Create and run the command line text using the toolchain
  sys_cmd <- paste(saga_cmd, "toolchains Satellite", 
                   "-DEM_REFERENCE", reference_raster,
                   "-INPUT", paste(satellite_files, collapse = ";")
  )
  system(sys_cmd)
  
  message("\nBands reprojected, starting satellite index calculations")
  
  reprojected_files <- list.files(satellite_out, full.names = TRUE)
  satellite_stack <- stack(reprojected_files)
  
  # Change band names to be more descriptive
  if(!is_empty(grep("*.b01$", names(satellite_stack)))) {
    names(satellite_stack[[grep("*.b01$", names(satellite_stack))]]) <- "ultrablue"
  }
  if(!is_empty(grep("*.b02$", names(satellite_stack)))) {
    names(satellite_stack[[grep("*.b02$", names(satellite_stack))]]) <- "blue"
  }
  if(!is_empty(grep("*.b03$", names(satellite_stack)))) {
    names(satellite_stack[[grep("*.b03$", names(satellite_stack))]]) <- "green"
  }
  if(!is_empty(grep("*.b04$", names(satellite_stack)))) {
    names(satellite_stack[[grep("*.b04$", names(satellite_stack))]]) <- "red"
  }
  if(!is_empty(grep("*.b05$", names(satellite_stack)))) {
    names(satellite_stack[[grep("*.b05$", names(satellite_stack))]]) <- "redEdge1"
  }
  if(!is_empty(grep("*.b06$", names(satellite_stack)))) {
    names(satellite_stack[[grep("*.b06$", names(satellite_stack))]]) <- "redEdge2"
  }
  if(!is_empty(grep("*.b07$", names(satellite_stack)))) {
    names(satellite_stack[[grep("*.b07$", names(satellite_stack))]]) <- "redEdge3"
  }
  if(!is_empty(grep("*.b08$", names(satellite_stack)))) {
    names(satellite_stack[[grep("*.b08$", names(satellite_stack))]]) <- "nir"
  }
  if(!is_empty(grep("*.b09$", names(satellite_stack)))) {
    names(satellite_stack[[grep("*.b09$", names(satellite_stack))]]) <- "wv"
  }
  if(!is_empty(grep("*.b11$", names(satellite_stack)))) {
    names(satellite_stack[[grep("*.b11$", names(satellite_stack))]]) <- "swir2"
  }
  if(!is_empty(grep("*.b12$", names(satellite_stack)))) {
    names(satellite_stack[[grep("*.b12$", names(satellite_stack))]]) <- "swir3"
  }
  if(!is_empty(grep("*.b8A$", names(satellite_stack)))) {
    names(satellite_stack[[grep("*.b8A$", names(satellite_stack))]]) <- "narrow_nir"
  }
  ...
  # Sets CRS, it was already transformed; this just allows for better CRS variable matching
  crs(satellite_indices) <- crs(raster(reference_raster)) 
  
  # Generare an RGB for background image
  RGB <- subset(satellite_stack, c("red", "green", "blue"))
  writeRaster(RGB, file.path(process_path, "sentinel2_RGB.tif"), overwrite = TRUE)
  ...
  if(save == TRUE){
    ...
    message("\nWriting ouputs")
    writeRaster(satellite_indices, 
                filename = file.path(covariate_file_path, 
                                     basename(satellite_out), 
...
```{r run function}
download_type <- "ee" # change to "scene" for a single scene, or "ee" for earth engine
if(download_type == "scene") {
  sat_indices <- bands_to_indices(
    reference_raster = dem_files[3], 
    satellite_files = list.files(file.path(raw_path, "2_extract"), full.names = TRUE), 
    single_scene = TRUE,
    process_path = file.path(raw_path, "3_reproject"),
    covariate_file_path = output_path, 
    save = TRUE, 
    remove_index = c("EVI2", "TVI"))
  
} else if (download_type == "ee") {
  season_dirs <- dir(file.path(raw_path, "1_download"), full.names = TRUE)[dir(file.path(raw_path, "1_download")) %in% seasons_df$season]
  for(i in season_dirs) {
    sat_indices <- bands_to_indices(
      reference_raster = dem_files[3], 
      satellite_files = list.files(i, full.names = TRUE),
      single_scene = FALSE,
      process_path = file.path(raw_path, "2_reproject"),
      covariate_file_path = output_path, 
      save = TRUE, 
      remove_index = c("EVI2"))
  }
}
plot(sat_indices$NDVI)
```